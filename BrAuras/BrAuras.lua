...
local ADDON_NAME = "BrAuras"
---@class BrAuras
local BrAuras = BrAuras
-- =========================
-- BanruoUI external refresh API (minimal)
-- Public wrappers used by BanruoUI when toggling load.never at runtime.
-- =========================
function BrAuras.ScanForLoads(toCheck, event, arg1, ...)
  if type(toCheck) == "string" then
    toCheck = {[toCheck] = true}
  end
  event = event or "BRAURAS_EXTERNAL_SCAN"
  Private.ScanForLoads(toCheck, event, arg1, ...)
end

function BrAuras.RebuildDisplays(ids)
  if type(ids) == "string" then
    ids = {[ids] = true}
  end
  if ids == nil then
    -- Fallback: re-evaluate everything
    BrAuras.ScanForLoads(nil, "BRAURAS_REBUILD")
    return
  end
  if type(ids) ~= "table" then return end

  -- Expand group/root ids to include their full controlledChildren subtree
  local expanded, seen = {}, {}
  local function ExpandSubtree(id)
  ...
  -- Unload any currently loaded displays in the affected set, and clear loaded cache
  local toUnload = {}
  for id in pairs(expanded) do
    if Private.loaded[id] then
      toUnload[id] = true
      Private.loaded[id] = nil
    end
  end

  if next(toUnload) then
    Private.UnloadDisplays(toUnload, "BRAURAS_REBUILD")
    Private.FinishLoadUnload()
  end

  -- Re-evaluate loads for the affected ids
  BrAuras.ScanForLoads(expanded, "BRAURAS_REBUILD")
end

function BrAuras.RebuildDisplay(id)
  if type(id) ~= "string" or id == "" then return end
  BrAuras.RebuildDisplays({[id] = true})
end
local L = BrAuras.L
local versionString = BrAuras.versionString
local prettyPrint = BrAuras.prettyPrint

BrAurasTimers = setmetatable({}, {__tostring=function() return "BrAuras" end})
LibStub("AceTimer-3.0"):Embed(BrAurasTimers)
...
function BrAurasTimers:ScheduleTimerFixed(func, delay, ...)
  if (delay < Private.maxTimerDuration) then
    if delay + GetTime() > maxUpTime then
      BrAuras.prettyPrint(BrAuras.L["Can't schedule timer with %i, due to a World of Warcraft bug with high computer uptime. (Uptime: %i). Please restart your computer."]:format(delay, GetTime()))
      return
    end
...
if CustomNames then
  BrAuras.GetName = CustomNames.Get
  BrAuras.UnitName = CustomNames.UnitName
  BrAuras.GetUnitName = CustomNames.GetUnitName
  BrAuras.UnitFullName = CustomNames.UnitFullName
else
  BrAuras.GetName = function(name) return name end
  BrAuras.UnitName = UnitName
  BrAuras.GetUnitName = GetUnitName
  BrAuras.UnitFullName = UnitFullName
end

local timer = BrAurasTimers
BrAuras.timer = timer

local loginQueue = {}
local queueshowooc

function BrAuras.InternalVersion()
...
do
  ...
  local function waErrorHandler(errorMessage)
    ...
    if currentErrorHandlerId then
      data = BrAuras.GetData(currentErrorHandlerId)
    ...
    table.insert(juicedMessage, L["BrAuras Version: %s"]:format(BrAuras.versionString))
...
function Private.LoadOptions(msg)
  if not(C_AddOns.IsAddOnLoaded("BrAurasOptions")) then
    if not BrAuras.IsLoginFinished() then
      prettyPrint(Private.LoginMessage())
      loginQueue[#loginQueue + 1] = BrAuras.OpenOptions
    elseif InCombatLockdown() then
      ...
      local loaded, reason = C_AddOns.LoadAddOn("BrAurasOptions");
      if not(loaded) then
        reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
        BrAuras.prettyPrint(string.format(L["Options could not be loaded, the addon is %s"], reason));
        return false;
      end
    end
  end
...
function BrAuras.OpenOptions(msg)
  if Private.NeedToRepairDatabase() then
    StaticPopup_Show("BRAURAS_CONFIRM_REPAIR", nil, nil, {reason = "downgrade"})
  elseif (BrAuras.IsLoginFinished() and Private.LoadOptions(msg)) then
    BrAuras.ToggleOptions(msg, Private);
  end
end
...
SLASH_BRAURAS1, SLASH_BRAURAS2 = "/brauras", "/bra";
function SlashCmdList.BRAURAS(input)
  local args, msg = {}, nil
  ...
  if msg == "pstart" then
    BrAuras.StartProfile(args[1]);
  elseif msg == "pstop" then
    BrAuras.StopProfile();
  elseif msg == "pprint" then
    BrAuras.PrintProfile();
  elseif msg == "pcancel" then
    BrAuras.CancelScheduledProfile()
  elseif msg == "pshow" or msg == "profiling" then
    BrAurasProfilingFrame:Toggle()
  elseif msg == "minimap" then
    BrAuras.ToggleMinimap();
  elseif msg == "help" then
    Private.PrintHelp();
  elseif msg == "repair" then
    StaticPopup_Show("BRAURAS_CONFIRM_REPAIR", nil, nil, {reason = "user"})
  elseif msg == "ff" or msg == "feat" or msg == "feature" then
    ...
    BrAuras.OpenOptions(msg);
  end
end

if not BrAuras.IsLibsOK() then return end

function BrAuras.ToggleMinimap()
  BrAurasSaved.minimap.hide = not BrAurasSaved.minimap.hide
  if BrAurasSaved.minimap.hide then
    LDBIcon:Hide("BrAuras");
    prettyPrint(L["Use /wa minimap to show the minimap icon again."])
  else
    LDBIcon:Show("BrAuras");
  end
end

BINDING_HEADER_BRAURAS = ADDON_NAME
BINDING_NAME_BRAURASTOGGLE = L["Toggle Options Window"]
BINDING_NAME_BRAURASPROFILINGTOGGLE = L["Toggle Performance Profiling Window"]
BINDING_NAME_BRAURASPRINTPROFILING = L["Print Profiling Results"]

-- An alias for BrAurasSaved, the SavedVariables
-- Noteable properties:
...
-- While true no events are handled. E.g. BrAuras is paused while the Options dialog is open
local paused = true;
...
BrAuras.raidUnits = {};
BrAuras.raidpetUnits = {};
BrAuras.partyUnits = {};
BrAuras.partypetUnits = {};
BrAuras.petUnitToUnit = {
  pet = "player"
}
BrAuras.unitToPetUnit = {
  player = "pet"
}
do
  for i=1,40 do
    BrAuras.raidUnits[i] = "raid"..i
    BrAuras.raidpetUnits[i] = "raidpet"..i
    BrAuras.petUnitToUnit["raidpet"..i] = "raid"..i
    BrAuras.unitToPetUnit["raid"..i] = "raidpet"..i
  end
  for i=1,4 do
    BrAuras.partyUnits[i] = "party"..i
    BrAuras.partypetUnits[i] = "partypet"..i
    BrAuras.petUnitToUnit["partypet"..i] = "party"..i
    BrAuras.unitToPetUnit["party"..i] = "partypet"..i
  end
end

---@param unit UnitToken
---@return boolean isPet
BrAuras.UnitIsPet = function(unit)
  return BrAuras.petUnitToUnit[unit] ~= nil
end
...
function BrAuras.RegisterSubRegionType(name, displayName, supportFunction, createFunction, modifyFunction, onAcquire, onRelease, default, addDefaultsForNewAura, properties, supportsAdd)
  if not(name) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - name is not a string", 2);
  elseif not(displayName) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - display name is not defined".." "..name, 2);
  elseif(type(displayName) ~= "string") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - display name is not a string", 2);
  elseif not(supportFunction) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - support function is not defined", 2);
  elseif(type(supportFunction) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - support function is not a function", 2);
  elseif not(createFunction) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - creation function is not a function", 2);
  elseif not(modifyFunction) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - modification function is not defined", 2);
  elseif(type(modifyFunction) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - modification function is not a function", 2)
  elseif not(onAcquire) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - onAcquire function is not defined", 2);
  elseif(type(onAcquire) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - onAcquire function is not a function", 2)
  elseif not(onRelease) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - onRelease function is not defined", 2);
  elseif(type(onRelease) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - onRelease function is not a function", 2)
  elseif not(default) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - default options are not defined", 2);
  elseif(type(default) ~= "table" and type(default) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - default options are not a table or a function", 2);
  elseif(addDefaultsForNewAura and type(addDefaultsForNewAura) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionType - addDefaultsForNewAura function is not nil or a function", 2)
  elseif(subRegionTypes[name]) then
    error("Improper arguments to BrAuras.RegisterSubRegionType - region type \""..name.."\" already defined", 2);
  else
...
function BrAuras.RegisterSubRegionOptions(name, createFunction, description, getAnchors)
  if not(name) then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - name is not a string", 2);
  elseif not(createFunction) then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - creation function is not a function", 2);
  elseif(getAnchors and type(getAnchors) ~= "function") then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - getAnchors function is not a function", 2);
  elseif(subRegionOptions[name]) then
    error("Improper arguments to BrAuras.RegisterSubRegionOptions - region type \""..name.."\" already defined", 2);
  else
...
---@diagnostic disable-next-line: duplicate-set-field (it's replaced in BrAurasOptions.lua)
function BrAuras.IsOptionsOpen()
  return false;
end
...
function BrAuras.GetActiveConditions(id, cloneId)
  ...
  if (data.actions) then
    if data.actions.init then
      if data.actions.init.do_custom and data.actions.init.custom then
        local func = BrAuras.LoadFunction("return function() "..(data.actions.init.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["init"] = func
      end
      if data.actions.init.do_custom_load and data.actions.init.customOnLoad then
        local func = BrAuras.LoadFunction("return function() "..(data.actions.init.customOnLoad).."\n end", data.id);
      ...
      if data.actions.init.do_custom_unload and data.actions.init.customOnUnload then
        local func = BrAuras.LoadFunction("return function() "..(data.actions.init.customOnUnload).."\n end", data.id);
    ...
    if (data.actions.start) then
      if (data.actions.start.do_custom and data.actions.start.custom) then
        local func = BrAuras.LoadFunction("return function() "..(data.actions.start.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["start"] = func;
      end

      if (data.actions.start.do_message and data.actions.start.message_custom) then
        local func = BrAuras.LoadFunction("return "..(data.actions.start.message_custom), data.id);
    ...
    if (data.actions.finish) then
      if (data.actions.finish.do_custom and data.actions.finish.custom) then
        local func = BrAuras.LoadFunction("return function() "..(data.actions.finish.custom).."\n end", data.id);
      ...
      if (data.actions.finish.do_message and data.actions.finish.message_custom) then
        local func = BrAuras.LoadFunction("return "..(data.actions.finish.message_custom), data.id);
...
local function CreateTalentCache()
  ...
  if BrAuras.IsClassicOrWrathOrCata() then
  ...
  elseif BrAuras.IsMists() then
    ...
    for tier = 1, MAX_TALENT_TIERS do
      for column = 1, NUM_TALENT_COLUMNS do
        ...
        -- Get the icon and name from the talent cache and record it in the table that will be used by BrAurasOptions
        if (talentName and talentIcon) then
...
-- when using this function, do not name your global data table "BrAurasCompanion"
function BrAuras.AddCompanionData(data)
  BrAuras.DeepMixin(Private.CompanionData, data)
end

-- add data from versions of companion compatible addon that does not use BrAuras.AddCompanionData yet
local function AddLegacyCompanionData()
  local CompanionData = BrAurasCompanion and BrAurasCompanion.BrAuras or BrAurasCompanion
  if CompanionData then
    BrAuras.AddCompanionData(CompanionData)
  end
end
...
function Private.PostAddCompanion()
  ...
  if count and count > 0 then
    BrAuras.prettyPrint(L["There are %i updates to your auras ready to be installed!"]:format(count))
  end
  -- nag if new installs
  if Private.CompanionData.stash and next(Private.CompanionData.stash) then
    BrAuras.prettyPrint(L["You have new auras ready to be installed!"])
  end
end
...
local function tooltip_draw(isAddonCompartment, blizzardTooltip)
  ...
  tooltip:ClearLines()
  tooltip:AddDoubleLine("BrAuras", versionString)
  ...
  if not BrAuras.IsOptionsOpen() then
    if paused then
...
BrAuras.GenerateTooltip = tooltip_draw;
...
local Broker_BrAuras;
Broker_BrAuras = LDB:NewDataObject("BrAuras", {
  type = "launcher",
  text = "BrAuras",
  icon = "Interface\\AddOns\\BrAuras\\Media\\Textures\\icon.blp",
  ...
  OnEnter = function(self)
    colorFrame:SetScript("OnUpdate", function(self, elaps)
      ...
      Broker_BrAuras.iconR = r + (r2 - r) * colorElapsed / colorDelay;
      Broker_BrAuras.iconG = g + (g2 - g) * colorElapsed / colorDelay;
      Broker_BrAuras.iconB = b + (b2 - b) * colorElapsed / colorDelay;
    end);
    local elapsed = 0;
    local delay = 1;
...
do -- Archive stuff (component removed in v2.6)
  -- BrAurasArchive addon has been removed intentionally.
  -- Keep a lightweight no-op archive interface so any history/migration calls do not error.
  local NullStore
  local function GetNullStore()
    if NullStore then return NullStore end
    NullStore = {
      stores = {},
      Validate = function(self) return true end,
      Get = function(self) return nil end,
      GetData = function(self) return nil end,
      Set = function(self) return nil end,
      Clean = function(self) return nil end,
      Drop = function(self) return nil end,
      Create = function(self) return nil end,
      Open = function(self) return nil end,
      Commit = function(self) return nil end,
      Close = function(self) return nil end,
      Delete = function(self) return nil end,
    }
    return NullStore
  end

  function BrAuras.LoadFromArchive(storeType, storeID)
    return GetNullStore()
  end
end
...
function BrAuras.IsLoginFinished()
    ...
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        ...
        if (guid and db.CurrentEncounter.boss_guids [guid]) then
          ...
          BrAuras.CurrentEncounter = db.CurrentEncounter
          return true
        end
      end
    end
...
function Private.Login(takeNewSnapshots)
  local loginThread = coroutine.create(function()
    ...
    coroutine.yield(100)
    if db.history then
      -- v2.6: Archive component removed; drop legacy inline history to reduce memory usage.
      db.history = nil
    end
    ...
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFBrAuras|r detected a corrupt entry in BrAuras saved displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
...
local BrAurasFrame = CreateFrame("Frame", "BrAurasFrame", UIParent);
Private.frames["BrAuras Main Frame"] = BrAurasFrame;
BrAurasFrame:SetAllPoints(UIParent);
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      LDBIcon:Register("BrAuras", Broker_BrAuras, db.minimap);
    end
...
function BrAuras.IsImporting()
  return importing;
end

function BrAuras.IsPaused()
  return paused;
end
...
function BrAuras.InLoadingScreen()
...
function Private.ResumeAllDynamicGroups(suspended)
  for _, id in ipairs(suspended) do
    local region = BrAuras.GetRegion(id)
...
local function StoreBossGUIDs()
  ...
  if (BrAuras.CurrentEncounter and BrAuras.CurrentEncounter.boss_guids) then
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        local guid = UnitGUID ("boss" .. i)
        if (guid) then
          BrAuras.CurrentEncounter.boss_guids [guid] = true
        end
      end
    end
    db.CurrentEncounter = BrAuras.CurrentEncounter
  end
  Private.StopProfileSystem("boss_guids")
end

local function DestroyEncounterTable()
  if (BrAuras.CurrentEncounter) then
    wipe(BrAuras.CurrentEncounter)
  end
  BrAuras.CurrentEncounter = nil
  db.CurrentEncounter = nil
end
...
local function CreateEncounterTable(encounter_id)
  ...
  return BrAuras.CurrentEncounter
end
...
function BrAuras.InstanceType()
  return (GetInstanceTypeAndSize())
end

---@return string difficulty
function BrAuras.InstanceDifficulty()
  return select(2, GetInstanceTypeAndSize())
end

---@return number? difficultyID
function BrAuras.InstanceTypeRaw()
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local encounter_id = BrAuras.CurrentEncounter and BrAuras.CurrentEncounter.id or 0
  ...
  if BrAuras.IsClassicOrWrathOrCataOrMists() then
    local raidID = UnitInRaid("player")
    if raidID then
      raidRole = select(10, GetRaidRosterInfo(raidID))
    end
    role = "none"
    if BrAuras.IsWrathClassic() then
  ...
  if BrAuras.IsClassicEra() then
    vehicle = UnitOnTaxi('player')
  end
  if BrAuras.IsWrathOrCataOrMistsOrRetail() then
  ...
  if (BrAuras.CurrentEncounter) then
    if (instanceId ~= BrAuras.CurrentEncounter.zone_id and not inCombat) then
      encounter_id = 0
      DestroyEncounterTable()
    end
  end
  ...
  -- Parent-group cascade for external toggles (BanruoUI root switching).
  -- If any ancestor group has load.never=true, all descendants are treated as not-loadable.
  local parentNeverCache = {}
  local function IsDisabledByParent(data)
    local cid = data.id
    local cached = parentNeverCache[cid]
    if cached ~= nil then
      return cached
    end

    local disabled = false
    for parentData in Private.TraverseParents(data) do
      if parentData and parentData.load and parentData.load.never then
        disabled = true
        break
      end
    end

    parentNeverCache[cid] = disabled
    return disabled
  end
  ...
  for id in pairs(toCheck) do
    ...
    if (data and not data.controlledChildren) then
      ...
      -- BanruoUI: treat load.never as a hard disable even if load.use_never is not set.
      if data.load and data.load.never then
      ...
      elseif BrAuras.IsWrathClassic() then
      ...
      elseif BrAuras.IsCataClassic() then
      ...
      -- BanruoUI: parent group load.never disables all descendants.
      if IsDisabledByParent(data) then
        shouldBeLoaded = false
        couldBeLoaded = false
      end
...
if BrAuras.IsWrathOrCataOrMists() then
  loadFrame:RegisterEvent("VEHICLE_UPDATE");
  loadFrame:RegisterEvent("UPDATE_VEHICLE_ACTIONBAR")
  loadFrame:RegisterEvent("UPDATE_OVERRIDE_ACTIONBAR");
end

if BrAuras.IsMists() then
  loadFrame:RegisterEvent("PET_BATTLE_OPENING_START");
  loadFrame:RegisterEvent("PET_BATTLE_CLOSE");
end
...
function Private.LoadDisplays(toLoad, ...)
  for id in pairs(toLoad) do
    local uid = BrAuras.GetData(id).uid
    ...
    if Private.DebugLog.IsEnabled(uid) then
      BrAuras.prettyPrint(L["Debug Logging enabled for '%s'"]:format(id))
...
function Private.GetDataByUID(uid)
  return BrAuras.GetData(UIDtoID[uid])
end
...
function BrAuras.Rename(data, newid)
  ...
  if(data.parent) then
    ...
    local parentRegion = BrAuras.GetRegion(data.parent)
...
function BrAuras.DeepMixin(dest, source)
...
function Private.NeedToRepairDatabase()
  return db.dbVersion and db.dbVersion > BrAuras.InternalVersion()
end
...
local function RepairDatabase()
  local coro = coroutine.create(function()
    ...
    db.dbVersion = BrAuras.InternalVersion()
...
StaticPopupDialogs["BRAURAS_CONFIRM_REPAIR"] = {
  text = "",
  button1 = L["Repair"],
  button2 = L["Cancel"],
  ...
  for _, data in pairs(db.displays) do
    if type(data.uid) == "string" then
      if seenUIDs[data.uid] then
        ...
        data.uid = BrAuras.GenerateUniqueID()
...
function Private.CheckForAnchorCycle(source)
  ...
  while source do
    cycle[source] = true
    local data = BrAuras.GetData(source)
    local target
    if data then
      if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame then
        if data.anchorFrameFrame:sub(1, 10) == "BrAuras:" then
...
function Private.AddMany(tbl, takeSnapshots)
  ...
  for _, data in ipairs(tbl) do
    ...
    if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame and data.anchorFrameFrame:sub(1, 10) == "BrAuras:" then
  ...
  for _, source in pairs(anchorTargets) do
    ...
    if Private.CheckForAnchorCycle(source) then
      BrAuras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(source))
  ...
  if takeSnapshots then
    ...
    if #order > 0 then
      Private.Threads:Add("snapshot", coroutine.create(function()
        prettyPrint(L["BrAuras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."])
      ...
      if next(BrAuras.LoadFromArchive("Repository", "migration").stores) ~= nil then
        C_Timer.After(1, function()
          prettyPrint(L["BrAuras has detected empty settings. If this is unexpected, ask for assitance on https://discord.gg/weakauras."])
        end)
      end
    end
  end
  ...
  for _, data in ipairs(order) do
    if data.parent and bads[data.parent] then
      ...
      local ok = xpcall(BrAuras.PreAdd, Private.GetErrorHandlerUid(data.uid, "PreAdd"), data, oldSnapshot)
      if not ok then
        prettyPrint(L["Unable to modernize aura '%s'. This is probably due to corrupt data or a bad migration, please report this to the BrAuras team."]:format(data.id))
    ...
    if not customOptionIsValid(option) or authorOptionKeys[option.key] then
      ...
      if optionClass == "simple" then
        if not option.key then
          option.key = BrAuras.GenerateUniqueID()
        end
...
function Private.ClearSounds(uid, severity)
  ...
  BrAuras.ClearAndUpdateOptions(data.id, true)
  BrAuras.FillOptions()
end

function BrAuras.PreAdd(data, snapshot)
  if not data then return end
    ...
    for _, subRegionData in ipairs(data.subRegions) do
      ...
      if subType and Private.subRegionTypes[subType] then
        if Private.subRegionTypes[subType].supports(data.regionType) then
          ...
          BrAuras.prettyPrint(L["ERROR in '%s' unknown or incompatible sub element type '%s'"]:format(data.id, subType))
        end
      end
    end
  end
...
local function cycleCheck(data)
  ...
  if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame and data.anchorFrameFrame:sub(1, 10) == "BrAuras:" then
    if Private.CheckForAnchorCycle(id) then
      BrAuras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(id))
...
function pAdd(data, simpleChange)
  local id = data.id;
  if not(id) then
    error("Improper arguments to BrAuras.Add - id not defined");
    return;
  end

  data.uid = data.uid or BrAuras.GenerateUniqueID()
  if db.displays[id] and db.displays[id].uid ~= data.uid then
    print("Improper? arguments to BrAuras.Add - id", id, "is assigned to a different uid.", data.uid, db.displays[id].uid)
  end
  if UIDtoID[data.uid] and UIDtoID[data.uid] ~= id then
    print("Improper? arguments to BrAuras.Add - uid is assigned to a id. Uid:", data.uid, "assigned too:", UIDtoID[data.uid], "assigning now to", data.id)
  end
  ...
  if simpleChange then
    db.displays[id] = data
    cycleCheck(data)
    if BrAuras.GetRegion(data.id) then
    ...
    if Private.IsGroupType(data) then
      ...
      if data.triggers.disjunctive == "custom" then
        triggerLogicFunc = BrAuras.LoadFunction("return "..(data.triggers.customTriggerLogic or ""), data.id);
      end
...
function BrAuras.Add(data, simpleChange)
  Private.TimeMachine:DestroyTheUniverse(data.id)
...
function Private.AddParents(data)
  ...
  if (parent) then
    local parentData = BrAuras.GetData(parent)
...
function Private.SetRegion(data, cloneId)
  ...
  if not(regionType) then
    ...
    if(not regionTypes[regionType]) then
      regionType = "fallback";
      print("Improper arguments to BrAuras.CreateRegion - regionType \""..data.regionType.."\" is not supported in ".. data.id)
    end
    ...
    if not(id) then
      ...
      if(cloneId) then
        ...
        if (not region or region.regionType ~= data.regionType) then
          ...
          if(clonePool[data.regionType] and clonePool[data.regionType][1]) then
            ...
            local clone = regionTypes[data.regionType].create(BrAurasFrame, data);
        ...
        if((not Private.regions[id]) or (not Private.regions[id].region) or Private.regions[id].regionType ~= regionType) then
          region = regionTypes[regionType].create(BrAurasFrame, data);
          Private.regions[id] = {
      ...
      local loginFinished = BrAuras.IsLoginFinished();
...
local function EnsureRegion(id)
  if not Private.regions[id] or not Private.regions[id].region then
    ...
    while(id) do
      ...
      if BrAuras.GetRegion(id) then
        break
      end
    end

    for _, toCreateId in ipairs_reverse(aurasToCreate) do
      local data = BrAuras.GetData(toCreateId)
      Private.SetRegion(data)
    end
  end
...
---@return table|nil
function BrAuras.GetRegion(id, cloneId)
  if(cloneId and cloneId ~= "") then
...
function Private.SetAllStatesHidden(id, triggernum)
  local triggerState = BrAuras.GetTriggerStateForTrigger(id, triggernum);
  local changed = false
...
function Private.ReleaseClone(id, cloneId, regionType)
  ...
  if region:IsProtected() then
    BrAuras.prettyPrint(L["Error '%s' created a secure clone. We advise deleting the aura. For more information:\nhttps://github.com/BrAuras/BrAuras2/wiki/Protected-Frames"]:format(id))
  else
...
do
  local function frame_monitor_callback(event, frame, unit, previousUnit)
      ...
      for region, data in pairs(glow_frame_monitor) do
        ...
        then
          if not new_frame then
            new_frame = BrAuras.GetUnitFrame(unit)
          end
    ...
    if type(anchor_unitframe_monitor) == "table" then
      for region, data in pairs(anchor_unitframe_monitor) do
        ...
        then
          if not new_frame then
            new_frame = BrAuras.GetUnitFrame(unit) or BrAuras.HiddenFrames
          end
          if new_frame ~= data.frame then
  ...
  LGF.RegisterCallback("BrAuras", "FRAME_UNIT_UPDATE", frame_monitor_callback)
  LGF.RegisterCallback("BrAuras", "FRAME_UNIT_ADDED", frame_monitor_callback)
  LGF.RegisterCallback("BrAuras", "FRAME_UNIT_REMOVED", frame_monitor_callback)
end
...
function Private.HandleGlowAction(actions, region)
  ...
  then
    ...
    if actions.glow_frame_type == "FRAMESELECTOR" then
      if actions.glow_frame:sub(1, 10) == "BrAuras:" then
        local frame_name = actions.glow_frame:sub(11)
        if BrAuras.GetData(frame_name) then
    ...
    elseif actions.glow_frame_type == "UNITFRAME" and region.state.unit then
      glow_frame = BrAuras.GetUnitFrame(region.state.unit)
      should_glow_frame = true
    elseif actions.glow_frame_type == "NAMEPLATE" and region.state.unit then
      glow_frame = BrAuras.GetUnitNameplate(region.state.unit)
      should_glow_frame = true
...
function Private.PerformActions(data, when, region)
  if (paused or BrAuras.IsOptionsOpen()) then
    return;
  end;
  local actions;
  local formatters
...
-- This has to be in BrAuras for now, because GetNameAndIcon can be called from the options
-- before the Options has access to Private
BrAuras.GetNameAndIcon = wrapTriggerSystemFunction("GetNameAndIcon", "nameAndIcon");
...
do
  local hiddenTooltip;
  function BrAuras.GetHiddenTooltip()
    if not(hiddenTooltip) then
      hiddenTooltip = CreateFrame("GameTooltip", "BrAurasTooltip", nil, "GameTooltipTemplate");
...
function BrAuras.GetAuraInstanceTooltipInfo(unit, auraInstanceId, filter)
  if BrAuras.IsRetail() then
    local tooltipText = ""
    local tooltipData
...
function BrAuras.GetAuraTooltipInfo(unit, index, filter)
  local tooltipText = ""
  if BrAuras.IsRetail() then
    ...
    local tooltip = BrAuras.GetHiddenTooltip();
    tooltip:ClearLines();
...
local FrameTimes = {};
function BrAuras.ProfileFrames(all)
  UpdateAddOnCPUUsage();
...
local DisplayTimes = {};
function BrAuras.ProfileDisplays(all)
  UpdateAddOnCPUUsage();
...
local function FixGroupChildrenOrderImpl(data, frameLevel)
  ...
  for _, childId in ipairs(data.controlledChildren) do
    local childData = BrAuras.GetData(childId)
...
function BrAuras.EnsureString(input)
  if (input == nil) then
    return "";
  end
...
function BrAuras.RegisterTriggerSystem(types, triggerSystem)
...
function BrAuras.RegisterTriggerSystemOptions(types, func)
  for _, v in ipairs(types) do
    Private.triggerTypesOptions[v] = func;
  end
end

function BrAuras.GetTriggerStateForTrigger(id, triggernum)
...
function BrAuras.GetActiveStates(id)
  return triggerState[id].activeStates
end

function BrAuras.GetActiveTriggers(id)
...
do
  ...
  function Private.UpdateFakeStatesFor(id)
    if (BrAuras.IsOptionsOpen() and visibleFakeStates[id]) then
      local data = BrAuras.GetData(id)
      if (data) then
        ...
        Private.UpdatedTriggerState(id)
        if BrAuras.GetMoverSizerId() == id then
    ...
    if (state.show) then
      ...
      for triggernum = -1, triggerState[id].numTriggers do
        ...
        if triggernum == activeTrigger then
          ...
          local triggerStates = BrAuras.GetTriggerStateForTrigger(id, triggernum)
...
function Private.UpdatedTriggerState(id)
  ...
  local activeTriggerState = BrAuras.GetTriggerStateForTrigger(id, newActiveTrigger);
...
local function ReplaceValuePlaceHolders(textStr, region, customCache, state, formatter, trigger)
  ...
  if customIndexSubStr then
    ...
    if custom then
      value = BrAuras.EnsureString(value)
    end
  else
...
function BrAuras.IsAuraActive(id)
  local active = triggerState[id]
...
local function ensureMouseFrame()
  ...
  ---@class Frame
  mouseFrame = CreateFrame("Frame", "BrAurasAttachToMouseFrame", UIParent);
  mouseFrame.attachedVisibleFrames = {};
  mouseFrame:SetWidth(1);
  mouseFrame:SetHeight(1);

  local moverFrame = CreateFrame("Frame", "BrAurasMousePointerFrame", mouseFrame);
...
function Private.ensurePRDFrame()
  ...
  personalRessourceDisplayFrame = CreateFrame("Frame", "BrAurasAttachToPRD", UIParent);
  ...
  local moverFrame = CreateFrame("Frame", "BrAurasPRDMoverFrame", personalRessourceDisplayFrame);
  ...
  texture:SetAllPoints(moverFrame);
  texture:SetTexture("Interface\\AddOns\\BrAuras\\Media\\Textures\\PRDFrame");
  ...
  personalRessourceDisplayFrame.OptionsOpened = function()
    ...
    if (KuiNameplatesCore and KuiNameplatesCore.profile) then
      ...
      personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\BrAuras\\Media\\Textures\\PRDFrameKui");
    else
      ...
      personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\BrAuras\\Media\\Textures\\PRDFrame");
    end
...
local function tryAnchorAgain()
  ...
  for id, _ in pairs(delayed) do
    ...
    if (data and region) then
      ...
      local parentData
      if data.parent then
        parentData = BrAuras.GetData(data.parent)
...
local HiddenFrames = CreateFrame("Frame", "BrAurasHiddenFrames")
HiddenFrames:Hide()
BrAuras.HiddenFrames = HiddenFrames
...
local function GetAnchorFrame(data, region, parent)
  ...
  if (anchorFrameType == "NAMEPLATE") then
    ...
    if unit then
      local frame = unit and BrAuras.GetUnitNameplate(unit)
  ...
  if (anchorFrameType == "UNITFRAME") then
    local unit = region.state and region.state.unit
    if unit then
      local frame = BrAuras.GetUnitFrame(unit) or BrAuras.HiddenFrames
      if frame then
  ...
  if (anchorFrameType == "SELECTFRAME" and anchorFrameFrame) then
    if(anchorFrameFrame:sub(1, 10) == "BrAuras:") then
  ...
  if (anchorFrameType == "CUSTOM" and region.customAnchorFunc) then
    ...
    if ok and frame then
      return frame
    elseif BrAuras.IsOptionsOpen() then
      return parent
    else
      return HiddenFrames
    end
...
function Private.AnchorFrame(data, region, parent, force)
  ...
  and not BrAuras.IsLoginFinished()
  and not force
  then
    ...
    if (data.anchorFrameParent or data.anchorFrameParent == nil
      ...
      region:SetParent(parent or BrAurasFrame);
    end
...
function BrAuras.SetModel(frame, unused, model_fileId, isUnit, isDisplayInfo)
...
function BrAuras.SafeToNumber(input)
...
function BrAuras.ReplaceRaidMarkerSymbols(txt)
  local start = 1
...
do
  ...
  function BrAuras.IsUntrackableSoftTarget(unit)
  ...
  ---@param unit UnitToken
  ---@return boolean result
  function BrAuras.UntrackableUnit(unit)
...
do
  ...
  ---@return string name
  ---@return string realm
  function BrAuras.UnitNameWithRealm(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm = UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end

  function BrAuras.UnitNameWithRealmCustomName(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm =  BrAuras.UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end
end
...
function BrAuras.IsAuraLoaded(id)
...
-- are categorized
---@private
function BrAuras.GetTriggerCategoryFor(triggerType)
...
function Private.SortOrderForValues(values)
  ...
  table.sort(sortOrder, function(aKey, bKey)
    ...
    if type(aValue) == "string" and aValue:sub(1, #BrAuras.newFeatureString) == BrAuras.newFeatureString then
      aValue = aValue:sub(#BrAuras.newFeatureString + 1)
    end

    if type(bValue) == "string" and bValue:sub(1, #BrAuras.newFeatureString) == BrAuras.newFeatureString then
      bValue = bValue:sub(#BrAuras.newFeatureString + 1)
    end
  ...
  local function Traverse(data, includeSelf, includeGroups, includeLeafs)
    ...
    if data.controlledChildren then
      for _, child in ipairs(data.controlledChildren) do
        Traverse(BrAuras.GetData(child), true, includeGroups, includeLeafs)
      end
    end
  end
