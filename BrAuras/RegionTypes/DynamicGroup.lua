...
function BrAuras.GetPolarCoordinates(x, y, originX, originY)
...
function BrAuras.InvertSort(sortFunc)
...
function BrAuras.SortNilLast(a, b)
...
local sortNilFirst = BrAuras.InvertSort(BrAuras.SortNilLast)
function BrAuras.SortNilFirst(a, b)
  if a == nil and b == nil then
...
function BrAuras.SortGreaterLast(a, b)
...
BrAuras.SortGreaterFirst = BrAuras.InvertSort(BrAuras.SortGreaterLast)

function BrAuras.SortRegionData(path, sortFunc)
  ...
  if type(sortFunc) ~= "function" then
    ...
    sortFunc = BrAuras.SortGreaterLast
  end
  return function(a, b)
...
function BrAuras.SortAscending(path)
  return BrAuras.SortRegionData(path, BrAuras.ComposeSorts(BrAuras.SortNilFirst, BrAuras.SortGreaterLast))
end

function BrAuras.SortDescending(path)
  return BrAuras.InvertSort(BrAuras.SortAscending(path))
end

function BrAuras.ComposeSorts(...)
...
local sorters = {
  none = function(data)
    return BrAuras.ComposeSorts(
      BrAuras.SortAscending({"dataIndex"}),
      BrAuras.SortAscending({"region", "state", "index"})
    ), { index = true }
  end,
  hybrid = function(data)
    ...
    if hybridSortAscending then
      sortExpirationTime = BrAuras.SortAscending({"region", "state", "expirationTime"})
    else
      sortExpirationTime = BrAuras.SortDescending({"region", "state", "expirationTime"})
    end
    return BrAuras.ComposeSorts(
      sortHybridStatus,
      sortExpirationTime,
  ...
  ascending = function(data)
    return BrAuras.ComposeSorts(
      BrAuras.SortAscending({"region", "state", "expirationTime"}),
      BrAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  descending = function(data)
    return BrAuras.ComposeSorts(
      BrAuras.SortDescending({"region", "state", "expirationTime"}),
      BrAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  custom = function(data)
    local sortStr = data.customSort or ""
    local sortFunc = BrAuras.LoadFunction("return " .. sortStr, data.id) or noop
    local sortOn = nil
    local events = BrAuras.split(data.sortOn or "")
    if #events > 0 then
      sortOn = {}
...
local anchorers = {
  ["NAMEPLATE"] = function(data)
    return function(frames, activeRegions)
      for _, regionData in ipairs(activeRegions) do
        ...
        local found
        if unit then
          local frame = BrAuras.GetUnitNameplate(unit)
          if frame then
        ...
        if not found and BrAuras.IsOptionsOpen() and regionData.region.state then
  ...
  ["CUSTOM"] = function(data)
    ...
    local anchorFunc = BrAuras.LoadFunction("return " .. anchorStr, data.id) or noop

    local anchorOn = nil
    local events = BrAuras.split(data.anchorOn or "")
    if #events > 0 then
      anchorOn = {}
      for _, event in ipairs(events) do
        anchorOn[event] = true
...
local growers = {
  ...
  CUSTOM = function(data)
    ...
    local growFunc = BrAuras.LoadFunction("return " .. growStr, data.id) or noop
    local growOn = nil
    local events = BrAuras.split(data.growOn or "")
    if #events > 0 then
      growOn = {}
      for _, event in ipairs(events) do
        growOn[event] = true
...
local function modify(parent, region, data)
  ...
  function region:IsSuspended()
    return not BrAuras.IsLoginFinished() or self.suspended > 0
  end
  ...
  function region:ReloadControlledChildren()
    ...
    if not self:IsSuspended() then
      ...
      for dataIndex, childID in ipairs(data.controlledChildren) do
        local childRegion, childData = BrAuras.GetRegion(childID), BrAuras.GetData(childID)
  ...
  function region:AddChild(childID, cloneID)
    ...
    local childRegion = BrAuras.GetRegion(childID, cloneID)
  ...
  function region:DoPositionChildrenPerFrame(frame, positions, handledRegionData)
    for regionData, pos in pairs(positions) do
      ...
      controlPoint:SetShown(show and frame ~= BrAuras.HiddenFrames)
      ...
      if animate then
        ...
        if show and (abs(xDelta) > 0.01 or abs(yDelta) > 0.01) then
          ...
          if data.grow == "CIRCLE" or data.grow == "COUNTERCIRCLE" then
            ...
            local radius1, previousAngle = BrAuras.GetPolarCoordinates(xPrev, yPrev, originX, originY)
            local radius2, newAngle = BrAuras.GetPolarCoordinates(x, y, originX, originY)
  ...
  function region:Resize()
    ...
    if not self:IsSuspended() then
      ...
      if isRestricted and not BrAuras.IsOptionsOpen() then
