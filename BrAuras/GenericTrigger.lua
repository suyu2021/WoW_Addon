...
#####################################################
# Helper functions mainly for the BrAuras Options #
#####################################################
...
function BrAuras.UnitExistsFixed(unit, smart)
...
function BrAuras.ScanEventsByID(event, id, ...)
...
function BrAuras.ScanEvents(event, arg1, arg2, ...)
      ...
      for id, triggers in pairs(event_list) do
        ...
        for triggernum, data in pairs(triggers) do
          ...
          then
            ...
            if delay == 0 then
              local allStates = BrAuras.GetTriggerStateForTrigger(id, triggernum);
...
function BrAuras.ScanUnitEvents(event, unit, ...)
...
function BrAuras.ScanEventsInternal(event_list, event, arg1, arg2, ... )
...
function Private.ScanEventsWatchedTrigger(id, watchedTriggernums)
  ...
  for _, watchedTrigger in ipairs(watchedTriggernums) do
    if watched_trigger_events[id] and watched_trigger_events[id][watchedTrigger] then
      local updatedTriggerStates = BrAuras.GetTriggerStateForTrigger(id, watchedTrigger)
      for observerTrigger in pairs(watched_trigger_events[id][watchedTrigger]) do
        ...
        local allstates = BrAuras.GetTriggerStateForTrigger(id, observerTrigger)
...
---@class BrAurasGenericTriggerFrame: FrameScriptObject
local frame = CreateFrame("Frame");
frame.unitFrames = {};
Private.frames["BrAuras Generic Trigger Frame"] = frame;
      ...
      if(Private.category_event_prototype[triggerType] or triggerType == "custom") then
        ...
        if(Private.category_event_prototype[triggerType]) then
          if not(trigger.event) then
            error("Improper arguments to BrAuras.Add - trigger type is \"event\" but event is not defined");
          elseif not(event_prototypes[trigger.event]) then
            if(event_prototypes["Health"]) then
              trigger.event = "Health";
            else
              error("Improper arguments to BrAuras.Add - no event prototype can be found for event type \""..trigger.event.."\" and default prototype reset failed.");
            end
          else
            ...
            elseif prototype.timedrequired then
              ...
              BrAuras.prettyPrint("Invalid Prototype found: " .. prototype.name)
            end
          ...
          triggerFunc = BrAuras.LoadFunction("return "..(trigger.custom or ""), data.id);
          if (trigger.custom_type == "stateupdate") then
            tsuConditionVariables = BrAuras.LoadFunction("return function() return \n" .. (trigger.customVariables or "") .. "\n end", data.id);
          ...
          if(trigger.custom_type == "status" or trigger.custom_type == "event" and trigger.custom_hide == "custom") then
            untriggerFunc = BrAuras.LoadFunction("return "..(untrigger.custom or ""), data.id);
          ...
          if(trigger.custom_type ~= "stateupdate" and trigger.customDuration and trigger.customDuration ~= "") then
            durationFunc = BrAuras.LoadFunction("return "..trigger.customDuration, data.id);
          end
          if(trigger.custom_type ~= "stateupdate") then
            overlayFuncs = {};
            for i = 1, 7 do
          ...
          if((trigger.custom_type == "status" or trigger.custom_type == "stateupdate") and trigger.check == "update") then
            ...
            local rawEvents = BrAuras.split(trigger.events);
...
do
  ...
  local ranged = BrAuras.IsClassicOrWrath() and GetInventorySlotInfo("RangedSlot")
  ...
  ---@return number? icon
  function BrAuras.GetSwingTimerInfo(hand)
    if(hand == "main") then
      ...
      elseif BrAuras.IsRetail() and lastSwingRange then
  ...
  ---@private
  function BrAuras.InitSwingTimer()
...
do
  ...
  local function CheckGCD()
    ...
    if(event and not BrAuras.IsPaused()) then
      Private.ScanEvents(event);
    end
  end
  ...
  local function HandleSpell(self, id, startTime, duration, modRate, paused)
    ...
    if duration > 0 then
      if (startTime == gcdStart and duration == gcdDuration)
          or (BrAuras.IsClassicOrWrath() and duration == shootDuration and startTime == shootStart)
      then
  ...
  local SpellDetails = {
    ...
    AddEffectiveSpellId = function(self, effectiveSpellId, userSpellId)
      ...
      spellDetail.known = BrAuras.IsSpellKnownIncludingPet(effectiveSpellId)
      ...
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
            ...
            = BrAuras.GetSpellCooldownUnified(effectiveSpellId, GetRuneDuration());
    ...
    CheckSpellKnown = function(self)
      ...
      for effectiveSpellId, spellDetailsData in pairs(self.data) do
        local known = BrAuras.IsSpellKnownIncludingPet(effectiveSpellId)
    ...
    CheckSpellCooldown = function(self, effectiveSpellId, runeDuration)
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
        startTimeCooldown, durationCooldown, cooldownBecauseRune, startTimeCharges, durationCharges,
        spellCount, unifiedModRate, modRate, modRateCharges, paused
        = BrAuras.GetSpellCooldownUnified(effectiveSpellId, runeDuration);
    ...
    WatchSpellCooldown = function(self, userSpellId, ignoreRunes, useExact, followoverride)
      ...
      if ignoreRunes and BrAuras.IsWrathOrCataOrMistsOrRetail() then
        for i = 1, 6 do
  ...
  function BrAuras.GetRuneCooldown(id)
  ...
  local essenceCache = {{},{},{},{},{},{}}
  ---@private
  function BrAuras.InitEssenceCooldown()
  ...
  function BrAuras.GetEssenceCooldown(essence)
  ...
  function BrAuras.GetSpellLossOfControlCooldown(identifier)
    if BrAuras.IsTWW() then
  ...
  function BrAuras.GetSpellCooldown(id, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
  ...
  function BrAuras.GetSpellCharges(id, ignoreSpellKnown)
  ...
  ---@return number gcdCooldown
  function BrAuras.GetItemCooldown(id, showgcd)
  ...
  ---@return number duration
  function BrAuras.gcdDuration()
    return gcdDuration or 0;
  end

  ---@return string name
  function BrAuras.GcdSpellName()
    return gcdSpellName;
  end
  ...
  ---@return boolean enabled
  ---@return number gcdCooldown
  function BrAuras.GetItemSlotCooldown(id, showgcd)
    local startTime, duration, enabled, gcdCooldown;
  ...
  function Private.CheckRuneCooldown()
    ...
    for id, _ in pairs(runes) do
      ...
      if(duration > 0 and duration ~= BrAuras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
  ...
  function BrAuras.GetSpellCooldownUnified(id, runeDuration)
  ...
  function Private.CheckItemCooldowns()
    for id, _ in pairs(items) do
      ...
      if(duration > 0 and duration > 1.5 and duration ~= BrAuras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
      ...
      if (itemCdEnabledChanged and not BrAuras.IsPaused()) then
        Private.ScanEventsByID("ITEM_COOLDOWN_CHANGED", id);
      end
    end
  end
  ...
  ---@private
  function BrAuras.WatchRuneCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
  ...
  ---@private
  function BrAuras.WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
    SpellDetails:WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
  end

  ---@private
  function BrAuras.WatchItemCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
    ...
    if not(items[id]) then
      ...
      itemCdEnabled[id] = enabled;
      if(duration and duration > 0 and duration > 1.5 and duration ~= BrAuras.gcdDuration()) then
        local time = GetTime();
        local endTime = startTime + duration;
        itemCdDurs[id] = duration;
        itemCdExps[id] = endTime;
  ...
  ---@private
  function BrAuras.WatchItemSlotCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
...
do
  ...
  ---@private
  function BrAuras.WatchSpellActivation(id)
    if (not id) then
      return;
    end
  ...
  function BrAuras.SpellActivationActive(id)
...
function BrAuras.GetPlayerReaction(unit)
...
---@param unit UnitToken
function BrAuras.WatchUnitChange(unit)
  unit = string.lower(unit)
  if not watchUnitChange then
    ...
    local function unitUpdate(unitA, eventsToSend)
      ...
      local newGUID = BrAuras.UnitExistsFixed(unitA) and UnitGUID(unitA)
    ...
    local function reactionUpdate(unit, eventsToSend)
      ...
      local newReaction = BrAuras.GetPlayerReaction(unit)
    ...
    local function reactionInit(unit)
      watchUnitChange.nameplateFaction[unit] = BrAuras.GetPlayerReaction(unit)
    end
    ...
    if BrAuras.IsClassicOrWrath() then
      function roleUpdate(unit, eventsToSend)
        ...
        local newRaidRole = BrAuras.UnitRaidRole(unit)
    ...
    local handleEvent = {
      ...
      UNIT_PET = function(unit, eventsToSend)
        local pet = BrAuras.unitToPetUnit[unit]
  ...
  watchUnitChange.unitIdToGUID[unit] = BrAuras.UnitExistsFixed(unit) and UnitGUID(unit)
...
function BrAuras.GetEquipmentSetInfo(itemSetName, partial)
...
function Private.WatchStagger()
  if not staggerWatchFrame then
    ...
    Private.frames["BrAuras Stagger Frame"] = staggerWatchFrame
...
if BrAuras.IsClassicOrWrath() then
  ...
  function BrAuras.WatchForQueuedSpell()
  ...
  function BrAuras.GetQueuedSpell()
...
---@return number? cost
function BrAuras.GetSpellCost(powerTypeToCheck)
  local spellID = select(9, BrAuras.UnitCastingInfo("player"))
  if BrAuras.IsClassicOrWrath() and not spellID then
    spellID = BrAuras.GetQueuedSpell()
  end
  if spellID then
...
-- Weapon Enchants
do
  local isCata = BrAuras.IsCataClassic()
  ...
  function BrAuras.TenchInit()
    if not(tenchFrame) then
      ...
      if BrAuras.IsRetail() then
        ...
        getTenchName = function(id)
          tenchTip = BrAuras.GetHiddenTooltip();
...
do
  ...
  ---@private
  function BrAuras.WatchForPetDeath()
...
do
  ...
  ---@private
  function BrAuras.WatchForCastLatency()
    if not castLatencyFrame then
      ...
      -- triggered from cacheEmpoweredFrame after updating cache use by BrAuras.UnitChannelInfo
  ...
  function BrAuras.GetCastLatency()
...
do
  ...
  ---@private
  function BrAuras.WatchForNameplateTargetChange()
...
do
  ...
  ---@private
  function BrAuras.WatchForPlayerMoving()
...
if C_AssistedCombat and C_AssistedCombat.GetNextCastSpell then
  ...
  ---@private
  function BrAuras.WatchForAssistedCombatNextCast()
...
---@private
function BrAuras.RegisterItemCountWatch()
...
function BrAuras.GetUniqueCloneId()
...
local commonConditions = {
  ...
  itemInRange = {
    display = BrAuras.newFeatureString .. L["Item in Range"],
    hidden = true,
    type = "bool",
    test = function(state, needle)
        ...
        if (enable) then
          ...
          if (result[v.name].type == "select" or result[v.name].type == "unit") then
            if (v.conditionValues) then
              result[v.name].values = Private[v.conditionValues] or BrAuras[v.conditionValues];
            else
              if type(v.values) == "function" then
                result[v.name].values = v.values()
              else
                result[v.name].values = Private[v.values] or BrAuras[v.values];
              end
            end
          end
...
do
  ...
  if class == "DEMONHUNTER"
    ...
    or class == "EVOKER"
  then
    function BrAuras.CalculatedGcdDuration()
...
BrAuras.CheckForItemBonusId = function(ids)
...
BrAuras.GetBonusIdInfo = function(ids, specificSlot)
...
BrAuras.CheckForItemEquipped = function(itemId, specificSlot)
...
BrAuras.GetCritChance = function()
...
Private.ExecEnv.GetCurrencyAccountInfo = function(currencyId)
  ...
  if BrAuras.IsRetail() and currencyInfo and currencyInfo.isAccountTransferable then
  ...
  -- WORKAROUND https://github.com/BrAuras/BrAuras2/issues/5106
  if currencyInfo and BrAuras.IsCataClassic() then
...
-- So for now make it simply a member of BrAuras
BrAuras.genericTriggerTypes = types

BrAuras.RegisterTriggerSystem(types, GenericTrigger);
