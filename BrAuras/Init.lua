...
BrAuras.normalWidth = 1.3
BrAuras.halfWidth = BrAuras.normalWidth / 2
BrAuras.doubleWidth = BrAuras.normalWidth * 2
local versionStringFromToc = C_AddOns.GetAddOnMetadata("BrAuras", "Version")
local versionString = "5.20.7"
local buildTime = "20251128202625"

local flavorFromToc = C_AddOns.GetAddOnMetadata("BrAuras", "X-Flavor")
...
if not versionString:find("beta", 1, true) then
  BrAuras.buildType = "release"
else
  BrAuras.buildType = "beta"
end

--[=[@alpha@
BrAuras.buildType = "alpha"
--@end-alpha@]=]

--[=====[@experimental@
BrAuras.buildType = "pr"
--@end-experimental@]=====]
...
if versionStringFromToc == "5.20.7" then
  ...
  buildTime = "Dev"
  BrAuras.buildType = "dev"
end
--@end-debug@]==]

BrAuras.versionString = versionStringFromToc
BrAuras.buildTime = buildTime
BrAuras.newFeatureString = "|TInterface\\OptionsFrame\\UI-OptionsFrame-NewFeatureIcon:0|t"
BrAuras.BuildInfo = select(4, GetBuildInfo())

---@return boolean result
function BrAuras.IsClassicEra()
  return flavor == 1
end
-- save compatibility with old auras
BrAuras.IsClassic = BrAuras.IsClassicEra

---@return boolean result
function BrAuras.IsWrathClassic()
  return flavor == 3
end

---@return boolean result
function BrAuras.IsCataClassic()
  return flavor == 4
end

---@return boolean result
function BrAuras.IsMists()
  return flavor == 5
end

---@return boolean result
function BrAuras.IsRetail()
  return flavor == 10
end

---@return boolean result
function BrAuras.IsClassicOrCata()
  return BrAuras.IsClassicEra() or BrAuras.IsCataClassic()
end

---@return boolean result
function BrAuras.IsClassicOrCataOrMists()
  return BrAuras.IsClassicOrCata() or BrAuras.IsMists()
end

---@return boolean result
function BrAuras.IsCataOrMists()
  return BrAuras.IsCataClassic() or BrAuras.IsMists()
end

function BrAuras.IsCataOrMistsOrRetail()
  return BrAuras.IsCataClassic() or BrAuras.IsMists() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsMistsOrRetail()
  return BrAuras.IsMists() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsCataOrRetail()
  return BrAuras.IsCataClassic() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsClassicOrWrath()
  return BrAuras.IsClassicEra() or BrAuras.IsWrathClassic()
end

---@return boolean result
function BrAuras.IsWrathOrCata()
  return BrAuras.IsWrathClassic() or BrAuras.IsCataClassic()
end

---@return boolean result
function BrAuras.IsWrathOrCataOrMists()
  return BrAuras.IsWrathClassic() or BrAuras.IsCataClassic() or BrAuras.IsMists()
end

---@return boolean result
function BrAuras.IsWrathOrCataOrMistsOrRetail()
  return BrAuras.IsWrathClassic() or BrAuras.IsCataClassic() or BrAuras.IsMists() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsClassicOrWrathOrCata()
  return BrAuras.IsClassicEra() or BrAuras.IsWrathClassic() or BrAuras.IsCataClassic()
end

---@return boolean result
function BrAuras.IsClassicOrWrathOrCataOrMists()
  return BrAuras.IsClassicEra() or BrAuras.IsWrathClassic() or BrAuras.IsCataClassic() or BrAuras.IsMists()
end

---@return boolean result
function BrAuras.IsWrathOrMistsOrRetail()
  return BrAuras.IsWrathClassic() or BrAuras.IsMists() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsWrathOrMists()
  return BrAuras.IsWrathClassic() or BrAuras.IsMists()
end

---@return boolean result
function BrAuras.IsWrathOrRetail()
  return BrAuras.IsWrathClassic() or BrAuras.IsRetail()
end

---@return boolean result
function BrAuras.IsTWW()
  return BrAuras.BuildInfo >= 110000
end

function BrAuras.IsMidnight()
  return BrAuras.BuildInfo >= 120000
end

---@param ... string
BrAuras.prettyPrint = function(...)
  print("|cff9900ffBrAuras:|r ", ...)
end

-- Force enable BrAurasCompanion and Archive because some addon managers interfere with it
C_AddOns.EnableAddOn("BrAurasCompanion")
...
do
    ...
    AddonCompartmentFrame:RegisterAddon({
      ...
      funcOnEnter = function(button)
        MenuUtil.ShowTooltip(button, function(tooltip)
          BrAuras.GenerateTooltip(true, tooltip)
        end)
      end,
  ...
  for _, lib in ipairs(StandAloneLibs) do
    if not lib then
        libsAreOk = false
        BrAuras.prettyPrint("Missing library:", lib)
    end
  end
  if LibStub then
...
function BrAuras.IsLibsOK()
  return libsAreOk
end

if not libsAreOk then
  C_Timer.After(1, function()
    BrAuras.prettyPrint("BrAuras is missing necessary libraries. Please reinstall a proper package.")
  end)
end

if BrAuras.IsWrathClassic() then
  C_Timer.After(1, function()
    BrAuras.prettyPrint("This version of BrAuras is provided as is. We are unable to test it ourselves on CN Servers.")
  end)
elseif BrAuras.IsMidnight() then
  C_Timer.After(1, function()
    BrAuras.prettyPrint("BrAuras does not support Midnight due to Blizzard restricting addons. Read more at https://patreon.com/BrAuras")
  end)
  libsAreOk = false
end

-- These function stubs are defined here to reduce the number of errors that occur if BrAuras.lua fails to compile
...
-- If BrAuras shuts down due to being installed on the wrong target, keep the bindings from erroring
--- @type fun(type: string)
---@diagnostic disable-next-line: duplicate-set-field
function BrAuras.StartProfile(_)
end

---@diagnostic disable-next-line: duplicate-set-field
function BrAuras.StopProfile()
end

---@diagnostic disable-next-line: duplicate-set-field
function BrAuras.PrintProfile()
end

function BrAuras.CountWagoUpdates()
