...
BrAurasProfilingReportMixin = {}
function BrAurasProfilingReportMixin:OnShow()
  ...
  self:SetTitle(L["BrAuras Profiling Report"])
  self:SetSize(500, 300)
end

function BrAurasProfilingReportMixin:ClearText()
  self.ScrollBox.messageFrame:SetText("")
end

function BrAurasProfilingReportMixin:AddText(v)
  if not v then
    return
  end
  --- @type string?
...
local RegisterProfile = function(startType)
  ...
  if startType == "encounter" then
    BrAurasProfilingFrame:UnregisterAllEvents()
    prettyPrint(L["Your next encounter will automatically be profiled."])
    BrAurasProfilingFrame:RegisterEvent("ENCOUNTER_START")
    BrAurasProfilingFrame:RegisterEvent("ENCOUNTER_END")
  ...
  elseif startType == "combat" then
    ...
    BrAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    BrAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    currentProfileState = startType
    delayedStart = true
  ...
  elseif startType and startType:match("%d") then
    ...
    ProfilingTimer = BrAuras.timer:ScheduleTimer(BrAuras.StopProfile, time)
    currentProfileState = "profiling"
  else
    BrAurasProfilingFrame:UnregisterAllEvents()
  ...
  BrAurasProfilingFrame:UpdateButtons()
  return delayedStart
end

---@diagnostic disable-next-line: duplicate-set-field
function BrAuras.StartProfile(startType)
...
function BrAuras.StopProfile()
  ...
  if BrAurasProfilingFrame then
    BrAurasProfilingFrame:UnregisterAllEvents()
    BrAurasProfilingFrame:UpdateButtons()
  end

  if ProfilingTimer then
    BrAuras.timer:CancelTimer(ProfilingTimer)
    ProfilingTimer = nil
  end
end

function BrAuras.ToggleProfile()
  if not profileData.systems.time or profileData.systems.time.count ~= 1 then
    BrAuras.StartProfile()
  else
    BrAuras.StopProfile()
  end
end
...
local function ColoredSpike(spike)
  local r, g, b
  if spike < 2 then
    r, g, b = BrAuras.GetHSVTransition(spike / 2, 0, 1, 0, 1, 1, 1, 0, 1)
  elseif spike < 2.5 then
    r, g, b = BrAuras.GetHSVTransition((spike - 2) * 2, 1, 1, 0, 1, 1, 0.65, 0, 1)
  elseif spike < 3 then
    r, g, b = BrAuras.GetHSVTransition((spike - 2.5) * 2, 1, 0.65, 0, 1, 1, 0, 0, 1)
  else
    r, g, b = 1, 0, 0
  end
...
function BrAuras.PrintProfile()
  local popup = BrAurasProfilingReport
  ...
  if BrAurasProfilingFrame and BrAurasProfilingFrame:IsShown() then
    popup:SetParent(BrAurasProfilingFrame)
    popup:SetPoint("TOPLEFT", BrAurasProfilingFrame, "TOPRIGHT", 5, 0)
  else
    popup:SetParent(UIParent)
    if BrAurasSaved.ProfilingWindow then
      popup:SetPoint("TOPLEFT", UIParent, "TOPLEFT", BrAurasSaved.ProfilingWindow.xOffset or 0, BrAurasSaved.ProfilingWindow.yOffset or 0)
    else
...
function BrAurasProfilingMixin:OnShow()
  ...
  ButtonFrameTemplate_HidePortrait(self)
  self:SetTitle(L["BrAuras Profiling"])
  ...
  self.buttons.report.tooltip = L["A detailed overview of your auras and BrAuras systems\nCopy the whole text to Weakaura's Discord if you need assistance."]
  ...
  view:SetElementInitializer("BrAurasProfilingLineTemplate", function(frame, elementData)
...
function BrAurasProfilingStopButton_OnClick(self)
  if currentProfileState == "profiling" then
    BrAuras.StopProfile()
  else
...
function BrAurasProfilingMixin:InitDropDown()
...
function BrAurasProfilingMixin:InitModeDropDown()
  local function Initializer(dropDown, level)
    for i = 1, 2 do
      local info = UIDropDownMenu_CreateInfo()
      info.text = modes[i]
...
local lastRefresh
function BrAurasProfilingMixin:RefreshBars(_, force)
...
function BrAurasProfilingMixin:ResetBars()
  self.bars:Flush()
end

function BrAurasProfilingMixin:UpdateBar(name, time, pct, spike)
  ...
  if elementData then
    ...
    local button = BrAurasProfilingFrame.ScrollBox:FindFrame(elementData)
    if button then
...
function BrAurasProfilingMixin:SortByColumnIndex(index)
...
function BrAurasProfilingMixin:UpdateButtons()
...
function BrAurasProfilingMixin:Start()
  self:Show()
end

function BrAurasProfilingMixin:Stop()
  BrAuras.StopProfile()
  self:UpdateButtons()
  self:ResetBars()
end

function BrAurasProfilingMixin:Toggle()
...
function BrAurasProfilingColumnDisplay_OnClick(self, columnIndex)
