...
function BrAuras.IsSpellInRange(spellId, unit)
  return SpellRange.IsSpellInRange(spellId, unit)
end

local LibRangeCheck = LibStub("LibRangeCheck-3.0")

function BrAuras.GetRange(unit, checkVisible)
  return LibRangeCheck:GetRange(unit, checkVisible);
end

function BrAuras.CheckRange(unit, range, operator)
...
function BrAuras.UnitDetailedThreatSituation(unit1, unit2)
...
BrAuras.UnitCastingInfo = UnitCastingInfo

if BrAuras.IsRetail() then
  local cacheEmpowered = {}
  BrAuras.UnitChannelInfo = function(unit)
  ...
  BrAuras.UnitChannelInfo = UnitChannelInfo
end

local constants = {
  nameRealmFilterDesc = L[" Filter formats: 'Name', 'Name-Realm', '-Realm'. \n\nSupports multiple entries, separated by commas\nCan use \\ to escape -."],
  instanceFilterDeprecated = L["This filter has been moved to the Location trigger. Change your aura to use the new Location trigger or join the BrAuras Discord server for help."],
...
if BrAuras.IsClassicOrWrathOrCataOrMists() then
  ...
  ---@return string? role
  BrAuras.UnitRaidRole = function(unit)
...
function BrAuras.SpellSchool(school)
  return Private.combatlog_spell_school_types[school] or ""
end

---@param flag integer
---@return integer index
function BrAuras.RaidFlagToIndex(flag)
...
---@return number b
---@return number a
function BrAuras.GetHSVTransition(perc, r1, g1, b1, a1, r2, g2, b2, a2)
...
straightHSV = [[
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    return BrAuras.GetHSVTransition(progress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    local angle = (progress * 2 * math.pi) - (math.pi / 2)
    local newProgress = ((math.sin(angle) + 1)/2);
    return BrAuras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
BrAuras.class_ids = {}
BrAuras.classes_sorted = {}
for classID = 1, 20 do -- GetNumClasses not supported by wow classic
  local classInfo = C_CreatureInfo.GetClassInfo(classID)
  if classInfo then
    BrAuras.class_ids[classInfo.classFile] = classInfo.classID
    tinsert(BrAuras.classes_sorted, classInfo.classFile)
  end
end
table.sort(BrAuras.classes_sorted)

if BrAuras.IsClassicOrWrathOrCata() then
  ---@param index integer
  ---@param extraOption boolean?
  ---@return boolean? hasTalent
  function BrAuras.CheckTalentByIndex(index, extraOption)
...
if BrAuras.IsRetail() then
  ...
  Private.frames["BrAuras talentCheckFrame"] = talentCheckFrame
  ...
  ---@return number? rank
  function BrAuras.GetTalentById(talentId)
  ...
  ---@param talentId integer
  ---@return boolean hasTalent
  function BrAuras.CheckTalentId(talentId)
...
---@return boolean hasTalent
---@return number? spellid
function BrAuras.CheckPvpTalentBySpellId(spellId)
...
function BrAuras.CheckNumericIds(loadids, currentId)
...
function BrAuras.ValidateNumeric(info, val)
...
function BrAuras.ValidateTime(info, val)
...
function BrAuras.TimeToSeconds(val)
...
---@param val string
---@return boolean result
function BrAuras.ValidateNumericOrPercent(info, val)
  if val ~= nil and val ~= "" then
...
---@return boolean result
function BrAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(spell)
...
---@private
function BrAuras.IsSpellKnownForLoad(spell, exact)
  if spell == 0 or spell >= 2^31 then return false end
  ...
  if (spellName) then
    ...
    if otherSpell and otherSpell ~= spell then
      return BrAuras.IsSpellKnownForLoad(otherSpell)
    end
  end
end
...
---@param pet boolean?
---@return boolean result
function BrAuras.IsSpellKnown(spell, pet)
  if spell == 0 or spell >= 2^31 then return false end
  if (pet) then
...
---@param spell string|number
---@return boolean result
function BrAuras.IsSpellKnownIncludingPet(spell)
  ...
  return BrAuras.IsSpellKnown(spell, false) or BrAuras.IsSpellKnown(spell, true)
end

if BrAuras.IsMists() then
  function BrAuras.IsGlyphActive(glyphID)
...
function BrAuras.GetNumSetItemsEquipped(setID)
...
---@return number result
function BrAuras.GetEffectiveAttackPower()
  local base, pos, neg = UnitAttackPower("player")
  return base + pos + neg
end

--- @type fun(): number
function BrAuras.GetEffectiveSpellPower()
...
local function valuesForTalentFunction(trigger)
  return function()
    ...
    elseif BrAuras.IsWrathOrCata() then
...
Private.load_prototype = {
  ...
  args = {
    ...
    {
      -- v2.6.2.4: hide load conditions: warmode / vehicle / encounter / petbattle / vehicleUi
      name = "warmode",
      display = L["In War Mode"],
      type = "tristate",
      init = BrAuras.IsRetail() and "arg" or nil,
      width = BrAuras.normalWidth,
      optional = true,
      enable = BrAuras.IsRetail(),
      hidden = true,
    ...
    },
    {
      ...
      type = "tristate",
      init = BrAuras.IsWrathClassic() and "arg" or nil,
      width = BrAuras.normalWidth,
      optional = true,
      enable = BrAuras.IsWrathClassic(),
      hidden = not BrAuras.IsWrathClassic(),
    ...
    },
    {
      ...
      type = "tristate",
      init = BrAuras.IsMistsOrRetail() and "arg" or nil,
      width = BrAuras.normalWidth,
      optional = true,
      enable = BrAuras.IsWrathOrMistsOrRetail(),
      hidden = true,
    ...
    {
      name = "vehicle",
      display = (BrAuras.IsClassicEra()) and L["On Taxi"] or L["In Vehicle"],
      type = "tristate",
      init = "arg",
      width = BrAuras.normalWidth,
      optional = true,
      hidden = true,
      events = (BrAuras.IsClassicEra()) and {"UNIT_FLAGS"}
    ...
    },
    {
      ...
      type = "tristate",
      init = BrAuras.IsWrathOrCataOrMistsOrRetail() and "arg" or nil,
      width = BrAuras.normalWidth,
      optional = true,
      enable = BrAuras.IsWrathOrCataOrMistsOrRetail(),
      hidden = true,
    ...
    },
    {
      ...
      display = L["Hardcore"],
      type = "tristate",
      init = BrAuras.IsClassicEra() and "arg" or nil,
      width = BrAuras.normalWidth,
      enable = BrAuras.IsClassicEra(),
      hidden = not BrAuras.IsClassicEra(),
    },
    {
      name = "engraving",
    ...
    },
    {
      ...
      type = "multiselect",
      values = "class_types",
      init = not BrAuras.IsRetail() and "arg" or nil,
      enable = not BrAuras.IsRetail(),
      hidden = BrAuras.IsRetail(),
    },
    {
      name = "class_and_spec",
      ...
      init = BrAuras.IsCataOrMistsOrRetail() and "arg" or nil,
      enable = BrAuras.IsCataOrMistsOrRetail(),
      hidden = not BrAuras.IsCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      test = BrAuras.IsRetail() and "BrAuras.CheckTalentId(%d) == (%d == 4)" or "BrAuras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
        if BrAuras.IsRetail() then
          ...
          return BrAuras.CheckTalentByIndex(talent, arg) ~= nil
        end
      end,
      multiConvertKey = BrAuras.IsRetail() and function(trigger, key)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
          local talentData = Private.GetTalentData(specId)
      ...
      events = (BrAuras.IsClassicOrWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE", "ACTIVE_TALENT_GROUP_CHANGED"})
        or (BrAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        -- Check for multi select!
        return BrAuras.IsClassicEra() and (load.talent_extraOption == 2 or load.talent_extraOption == 3)
      end,
      extraOption = BrAuras.IsClassicEra() and {
        display = "",
        values = function()
          return Private.talent_extra_option_types
        end
      },
      control = BrAuras.IsWrathOrCataOrMistsOrRetail() and "BrAurasMiniTalent" or nil,
      multiNoSingle = BrAuras.IsWrathOrCataOrMistsOrRetail(), -- no single mode
      multiTristate = BrAuras.IsWrathOrCataOrMistsOrRetail(), -- values can be true/false/nil
      multiAll = BrAuras.IsWrathOrCataOrMistsOrRetail(), -- require all tests
      orConjunctionGroup = BrAuras.IsWrathOrCataOrMistsOrRetail() and "talent",
      multiUseControlWhenFalse = BrAuras.IsWrathOrCataOrMistsOrRetail(),
      enable = function(trigger)
        return BrAuras.IsClassicEra()
            or (BrAuras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (BrAuras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
      hidden = function(trigger)
        return not (
            BrAuras.IsClassicEra()
            or (BrAuras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (BrAuras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil))
      end,
    },
    {
      name = "talent2",
      display = BrAuras.IsWrathOrCataOrMistsOrRetail() and L["Or Talent"] or L["And Talent"],
      type = "multiselect",
      values = valuesForTalentFunction,
      test = BrAuras.IsRetail() and "BrAuras.CheckTalentId(%d) == (%d == 4)" or "BrAuras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
      ...
      events = (BrAuras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        or (BrAuras.IsWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE"})
        or (BrAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return BrAuras.IsClassicEra() and (load.talent2_extraOption == 2 or load.talent2_extraOption == 3)
      end,
      extraOption = BrAuras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      events = (BrAuras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        ...
        or (BrAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return BrAuras.IsClassicEra() and (load.talent3_extraOption == 2 or load.talent3_extraOption == 3)
      end,
      extraOption = BrAuras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      test = "BrAuras.CheckTalentId(%d) == (%d == 4)",
      enableTest = function(trigger, talent, arg)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
      ...
      enable = function(trigger)
        return BrAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil
      end,
      hidden = function(trigger)
        return not (BrAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
    },
    {
      -- v2.6.2.5: hide load conditions: PvP talent / spell known / name realm / guild
      name = "pvptalent",
      ...
      test = "BrAuras.CheckPvpTalentBySpellId(%d)",
      enable = BrAuras.IsRetail(),
      hidden = not BrAuras.IsRetail(),
    ...
    },
    {
      ...
      test = "BrAuras.IsGlyphActive(%s)",
      events = {"GLYPH_ADDED", "GLYPH_REMOVED", "GLYPH_UPDATED", "USE_GLYPH"},
      enable = BrAuras.IsMists(),
      hidden = not BrAuras.IsMists(),
    },
    {
      name = "spellknown",
      ...
      type = "spell",
      hidden = true,
      test = "BrAuras.IsSpellKnownForLoad(%s, %s)",
      events = BrAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
      name = "not_spellknown",
      display = BrAuras.newFeatureString .. L["|cFFFF0000Not|r Spell Known"],
      type = "spell",
      hidden = true,
      test = "not BrAuras.IsSpellKnownForLoad(%s, %s)",
      events = BrAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
    ...
    {
      name = "spec_position",
      display = BrAuras.newFeatureString .. L["Spec Position"],
      type = "multiselect",
    ...
    },
    {
      ...
      init = BrAuras.IsClassicOrWrathOrCataOrMists() and "arg" or nil,
      enable = BrAuras.IsClassicOrWrathOrCataOrMists(),
      hidden = BrAuras.IsRetail(),
    ...
    {
      name = "group_leader",
      display = BrAuras.newFeatureString .. L["Group Leader/Assist"],
      type = "multiselect",
      init = "arg",
      hidden = true,
    ...
    },
    {
      ...
      test = "BrAuras.CheckNumericIds(%q, encounterid)",
      events = {"ENCOUNTER_START", "ENCOUNTER_END"},
      optional = true,
    },
    {
    ...
    },
    {
      ...
      init = not BrAuras.IsClassicEra() and "arg" or nil,
      enable = not BrAuras.IsClassicEra(),
      hidden = true,
    ...
    },
    {
      ...
      display = BrAuras.newFeatureString .. L["|cFFFF0000Not|r Item Equipped"],
      type = "item",
      hidden = true,
      multiEntry = {
        operator = "or"
    ...
    },
    {
      ...
      type = "string",
      hidden = true,
      test = "BrAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    },
    {
      name = "not_item_bonusid_equipped",
      display =  BrAuras.newFeatureString .. L["|cFFFF0000Not|r Item Bonus Id Equipped"],
      type = "string",
      hidden = true,
      test = "not BrAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    }
  }
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        ...
        if (includePets ~= nil and isPet) or (includePets ~= "PetsOnly" and not isPet) then
          if unitisunit then
            BrAuras.WatchUnitChange(unitisunit)
          end
          BrAuras.WatchUnitChange(unit)
        end
      end
    else
      if unitisunit then
      ...
      BrAuras.WatchUnitChange(triggerUnit)
    end
  end
end
...
local function AddUnitSpecChangeInternalEvents(triggerUnit, t)
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      local isPet = BrAuras.UnitIsPet(unit)
      if (not isPet) then
...
local function AddUnitRoleChangeInternalEvents(triggerUnit, t)
  ...
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      if not BrAuras.UnitIsPet(unit) then
        tinsert(t, "UNIT_ROLE_CHANGED_" .. string.lower(unit))
      end
    end
  else
    if not BrAuras.UnitIsPet(triggerUnit) then
...
Private.event_prototypes = {
  ["Unit Characteristics"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local smart = %s
        local extraUnit = %q;
        local name, realm = BrAuras.UnitNameWithRealm(unit)
      ]=];
    ...
    args = {
      ...
      },
      {
        ...
        type = "multiselect",
        init = "BrAuras.SpecForUnit(unit)",
        values = "spec_types_all",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return BrAuras.IsCataOrMistsOrRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end,
      ...
      },
      {
        ...
        conditionType = "select",
        enable = BrAuras.IsWrathOrMistsOrRetail(),
        hidden = not BrAuras.IsWrathOrMistsOrRetail(),
      },
      {
        name = "creatureType",
      ...
      },
      {
        ...
        sorted = true,
        conditionType = "select",
        enable = BrAuras.IsMistsOrRetail(),
        hidden = not BrAuras.IsMistsOrRetail(),
      },
      {
        name = "creatureFamily",
      ...
      },
      {
        ...
        display = L["Raid Role"],
        type = "select",
        init = "BrAuras.UnitRaidRole(unit)",
        values = "raid_role_types",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return BrAuras.IsClassicOrWrathOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      {
        name = "raidMarkIndex",
      ...
      },
      {
        ...
        width = BrAuras.doubleWidth,
        enable = function(trigger)
          return BrAuras.IsRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end,
      ...
      },
      {
        ...
        display = L["Hostility"],
        type = "select",
        init = "BrAuras.GetPlayerReaction(unit)",
      ...
      {
        hidden = true,
        test = "BrAuras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
  ...
  ["Faction Reputation"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = BrAuras.IsRetail() and function(trigger)
  ...
  ["Health"] = {
    ...
    events = function(trigger)
      local unit = trigger.unit
      local result = {}
      if BrAuras.IsClassicOrWrath() then
    ...
    args = {
      ...
      {
        name = "maxhealth",
        display = BrAuras.newFeatureString .. L["Max Health"],
        type = "number",
        init = "total",
        store = true,
        conditionType = "number",
      ...
      },
      {
        ...
        values = "absorb_modes",
        required = true,
        enable = function(trigger) return BrAuras.IsMistsOrRetail() and trigger.use_showAbsorb end,
        hidden = not BrAuras.IsMistsOrRetail()
      },
      {
        name = "showHealAbsorb",
      ...
      },
      {
        ...
        values = "absorb_modes",
        required = true,
        enable = function(trigger) return BrAuras.IsMistsOrRetail() and trigger.use_showHealAbsorb end,
        hidden = not BrAuras.IsMistsOrRetail()
      },
      {
        name = "absorb",
    ...
    overlayFuncs = {
      {
        ...
        enable = function(trigger)
          return BrAuras.IsMistsOrRetail() and trigger.use_showAbsorb;
        end
      },
      {
        name = L["Heal Absorb"],
        ...
        enable = function(trigger)
          return BrAuras.IsMistsOrRetail() and trigger.use_showHealAbsorb;
        end
      },
      {
  ...
  ["Power"] = {
    ...
    internal_events = function(trigger)
      ...
      if BrAuras.IsClassicOrWrath() and trigger.use_showCost and trigger.unit == "player" then
        tinsert(result, "WA_UNIT_QUEUED_SPELL_CHANGED");
      end
      if BrAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26 then
        tinsert(result, "ECLIPSE_DIRECTION_CHANGE");
      end
      if BrAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 4 then
    ...
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        if not BrAuras.IsRetail() and powerType == 99 then powerType = 1 end
      ...
      elseif BrAuras.IsMists() and powerType == 99 then
        table.insert(ret, ([[
          local power = UnitStagger(unit) or 0
          local scaleStagger = %s
          local total = math.max(1, UnitHealthMax(unit) * scaleStagger)
        ]]):format(trigger.use_scaleStagger and trigger.scaleStagger or 1))
      elseif BrAuras.IsMists() and (powerType == 14 or powerType == 7) then
        table.insert(ret, [[
      ...
      if (canEnableShowCost and trigger.use_showCost) then
        table.insert(ret, [[
          if (event == "UNIT_DISPLAYPOWER") then
            local cost = BrAuras.GetSpellCost(powerTypeToCheck)
    ...
    args = {
      ...
      },
      {
        ...
        reloadOptions = true,
        enable = function(trigger)
          return BrAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4
        end,
        hidden = not BrAuras.IsRetail()
      },
      {
      ...
      },
      {
        ...
        store = true,
        enable = function(trigger)
          return BrAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26
        end,
      },
      {
        name = "power",
      ...
      {
        name = "maxpower",
        display = BrAuras.newFeatureString .. L["Max Power"],
        type = "number",
        init = "total",
        store = true,
        conditionType = "number",
      ...
      },
      {
        ...
        conditionType = "select",
        enable = function(trigger)
          return BrAuras.IsClassicOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      {
        name = "raidMarkIndex",
    ...
    overlayFuncs = {
      ...
      },
      {
        ...
        enable = function(trigger)
          return BrAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4 and trigger.use_showChargedComboPoints
        end,
      },
    },
    automaticrequired = true
  ...
  ["Alternate Power"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        unit = string.lower(unit)
        local unitname, realm = BrAuras.UnitNameWithRealm(unit)
        local smart = %s
      ]=]
    ...
    args = {
      ...
      {
        hidden = true,
        test = "name and BrAuras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ["Combat Log"] = {
    ...
    args = {
      ...
      },
      {
        ...
        display = BrAuras.newFeatureString .. L["Source unit's raid mark index"],
        init = "BrAuras.RaidFlagToIndex(sourceRaidFlags)",
        test = "true",
        store = true,
        hidden = true,
      },
      {
        name = "sourceRaidMark",
        display = BrAuras.newFeatureString .. L["Source unit's raid mark texture"],
        test = "true",
      ...
      },
      {
        ...
        display = BrAuras.newFeatureString .. L["Destination unit's raid mark index"],
        init = "BrAuras.RaidFlagToIndex(destRaidFlags)",
        test = "true",
        store = true,
        hidden = true,
        enable = function(trigger)
      ...
      {
        name = "destRaidMark",
        display = BrAuras.newFeatureString .. L["Destination unit's raid mark texture"],
        test = "true",
      ...
      {
        name = "spellSchool",
        display = BrAuras.newFeatureString .. L["Spell School"],
        type = "select",
      ...
      {
        name = "extraSpellId",
        display = BrAuras.newFeatureString .. L["Extra Spell Id"],
        init = "arg",
        enable = function(trigger)
        ...
        test = BrAuras.IsClassicEra() and "GetSpellInfo(%q or '') == extraSpellName" or nil,
        type = "spell",
        showExactOption = false,
        store = true,
        conditionType = "number",
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        init = "use_cloneId and BrAuras.GetUniqueCloneId() or ''"
      },
      {
        hidden = true,
        name = "icon",
  ...
  ["Spell Activation Overlay"] = {
    ...
    loadFunc = function(trigger)
      if (trigger.use_exact_spellName) then
        BrAuras.WatchSpellActivation(tonumber(trigger.spellName));
      else
        BrAuras.WatchSpellActivation(type(trigger.spellName) == "number" and Private.ExecEnv.GetSpellName(trigger.spellName) or trigger.spellName);
      end
    end,
    init = function(trigger)
      local spellName
    ...
    args = {
      ...
      {
        hidden = true,
        test = "BrAuras.SpellActivationActive(spellName)";
      }
    },
    automaticrequired = true,
    progressType = "static"
  ...
  ["Cooldown Progress (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      BrAuras.WatchSpellCooldown(spellName, trigger.use_matchedRune, exactMatch, followoverride)
      if (trigger.use_showgcd) then
        BrAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        local name, _, icon = Private.ExecEnv.GetSpellInfo(effectiveSpellId)
        local startTime, duration, gcdCooldown, readyTime, modRate, paused = BrAuras.GetSpellCooldown(effectiveSpellId, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = BrAuras.GetSpellCharges(effectiveSpellId, ignoreSpellKnown)
        ...
        if showlossofcontrol and startTime and duration then
          local locStart, locDuration = BrAuras.GetSpellLossOfControlCooldown(spellname);
    ...
    args = {
      ...
      },
      {
        ...
        display = BrAuras.newFeatureString .. L["Show Loss of Control"],
        type = "toggle",
        test = "true",
        collapse = "extra Cooldown Progress (Spell)",
      },
      ...
      },
      {
        ...
        conditionType = "bool",
        conditionTest = function(state, needle)
          return state and state.show and (UnitExists('target') and state.spellname and BrAuras.IsSpellInRange(state.spellname, 'target') == needle)
        end,
        conditionEvents = AddTargetConditionEvents({
          "WA_SPELL_RANGECHECK",
        }),
  ...
  ["Cooldown Ready (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      local followoverride = not trigger.use_ignoreoverride
      BrAuras.WatchSpellCooldown(spellName, false, useExact, followoverride)
    end,
    init = function(trigger)
      local spellName = type(trigger.spellName) ~= "table" and trigger.spellName or 0
      if type(trigger.spellName) == "string" then
  ...
  ["Cooldown Progress (Item)"] = {
    ...
    loadFunc = function(trigger)
      BrAuras.WatchItemCooldown(trigger.itemName or 0)
      if (trigger.use_showgcd) then
        BrAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local showgcd = %s
        local startTime, duration, enabled, gcdCooldown = BrAuras.GetItemCooldown(itemname, showgcd);
    ...
    args = {
      ...
      {
        name = "itemId",
        display = BrAuras.newFeatureString .. L["ItemId"],
        hidden = true,
        init = "itemId",
        test = "true",
        store = true,
  ...
  ["Cooldown Progress (Equipment Slot)"] = {
    ...
    loadFunc = function(trigger)
      BrAuras.WatchItemSlotCooldown(trigger.itemSlot);
      if (trigger.use_showgcd) then
        BrAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      local ret = [[
        local showgcd = %s
        local itemSlot = %s
        local startTime, duration, enable, gcdCooldown = BrAuras.GetItemSlotCooldown(itemSlot, showgcd)
        local expirationTime = startTime + duration
        local genericShowOn = %s
  ...
  ["Global Cooldown"] = {
    ...
    init = function(trigger)
      local ret = [[
        local inverse = %s;
        local duration, expirationTime, name, icon = BrAuras.GetGCDInfo()
        local hasSpellName = BrAuras.GcdSpellName();
      ]];
  ...
  ["Swing Timer"] = {
    ...
    init = function(trigger)
      local ret = [=[
        ...
        local duration, expirationTime, name, icon = BrAuras.GetSwingTimerInfo(hand)
  ...
  ["Action Usable"] = {
    ...
    init = function(trigger)
      ...
      if trigger.use_ignoreSpellCooldown then
        ...
        ret = ret .. [=[
        local startTime, duration, gcdCooldown, readyTime, paused = BrAuras.GetSpellCooldown(effectiveSpellId, nil, nil, nil, nil)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = BrAuras.GetSpellCharges(effectiveSpellId, nil)
      ...
      if(trigger.use_targetRequired) then
        ret = ret.."active = active and BrAuras.IsSpellInRange(spellName or '', 'target')\n";
      end
  ...
  ["Talent Known"] = {
    ...
    events = function()
      ...
      elseif BrAuras.IsWrathOrCataOrMists() then
        events = {
          "CHARACTER_POINTS_CHANGED",
          "SPELLS_CHANGED",
    ...
    internal_events = BrAuras.IsRetail() and  {"WA_TALENT_UPDATE"} or nil,
    force_events = (BrAuras.IsRetail() and "TRAIT_CONFIG_UPDATED") or "CHARACTER_POINTS_CHANGED",
    name = L["Talent Known"],
    init = function(trigger)
      local inverse = trigger.use_inverse and not BrAuras.IsMistsOrRetail()
      local ret = {}
          ...
          for index, value in pairs(trigger.talent.multi) do
            ...
            elseif BrAuras.IsRetail() then
              table.insert(ret, ([[
                ...
                local shouldBeActive = %s
                if talentId then
                  activeName, activeIcon, _, rank = BrAuras.GetTalentById(talentId)
      ...
      if (BrAuras.IsTWW() and trigger.use_herotalent == false) then
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        init = "BrAuras.IsRetail() and Private.ExecEnv.GetSpecialization()",
        required = true,
        values = function(trigger)
          return BrAuras.spec_types_specific[trigger.class]
        end,
        enable = function(trigger)
          if BrAuras.IsRetail() and trigger.use_class and trigger.class then
            return true
          else
            return false
          end
      ...
      },
      {
        ...
        multiUseControlWhenFalse = BrAuras.IsMistsOrRetail(),
        multiAll = BrAuras.IsMistsOrRetail(),
        multiNoSingle = BrAuras.IsMistsOrRetail(),
        multiTristate = BrAuras.IsMistsOrRetail(), -- values can be true/false/nil
        control = BrAuras.IsMistsOrRetail() and "BrAurasMiniTalent" or nil,
        multiConvertKey = BrAuras.IsRetail() and function(trigger, key)
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
      ...
      },
      {
        ...
        type = "multiselect",
        values = function(trigger)
          if not BrAuras.IsTWW() then return {} end
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
              classId = i
        ...
        enable = function(trigger)
          return BrAuras.IsTWW() and trigger.use_class and trigger.class and trigger.use_spec and trigger.spec and true or false
        end,
        test = "heroActive",
        reloadOptions = true,
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = not BrAuras.IsMistsOrRetail(),
        hidden = BrAuras.IsMistsOrRetail(),
      },
      {
        hidden = true,
        name = "icon",
    ...
    init = function(trigger)
      ...
      if (trigger.use_talent) then
        ...
        table.insert(ret, ([[
          ...
          local active, spellId = BrAuras.CheckPvpTalentBySpellId(index)
          if spellId then
        ...
        if (trigger.talent.multi) then
          ...
          for index in pairs(trigger.talent.multi) do
            table.insert(ret, ([[
              if (not active) then
                local index = %s
                active, spellId = BrAuras.CheckPvpTalentBySpellId(index)
  ...
  ["Totem"] = {
    ...
    triggerFunction = function(trigger)
      ...
      ret = ret:format(trigger.use_totemType and tonumber(trigger.totemType) or "nil",
        ...
        BrAuras.IsRetail() and trigger.use_totemSpellId and trigger.totemSpellId or "nil",
        BrAuras.IsRetail() and not trigger.use_ignoreoverride and "true" or "false",
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return trigger.use_totemSpellId and BrAuras.IsRetail()
        end,
      },
      {
        name = "icon",
  ...
  ["Item Count"] = {
    ...
    loadFunc = function(trigger)
      if(trigger.use_includeCharges) then
        BrAuras.RegisterItemCountWatch();
      end
    end,
    init = function(trigger)
      local itemName = type(trigger.itemName) == "number" and trigger.itemName or string.format("%q", trigger.itemName or "0")
      local ret = [[
        ...
        if BrAuras.IsRetail() and itemName then
      ...
      return ret:format(
        ...
        BrAuras.IsRetail() and trigger.use_includeWarBandBank and "true" or "nil"
      )
    end,
    args = {
      {
  ...
  ["Stance/Form/Aura"] = {
    ...
    nameFunc = function(trigger)
      ...
      if(class == trigger.class) then
        ...
        local types = BrAuras[class:lower().."_form_types"];
        if(types) then
  ...
  ["Weapon Enchant"] = {
    ...
    name = BrAuras.IsRetail() and L["Weapon Enchant / Fishing Lure"] or L["Weapon Enchant"],
    init = function(trigger)
      BrAuras.TenchInit();
      ...
      local ret = [[
        ...
        if triggerWeaponType == "main" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = BrAuras.GetMHTenchInfo()
        elseif triggerWeaponType == "off" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = BrAuras.GetOHTenchInfo()
        elseif triggerWeaponType == "ranged" and BrAuras.IsCataClassic() then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = BrAuras.GetRangeTenchInfo()
        end
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return not BrAuras.IsRetail() and (not trigger.showOn or trigger.showOn == "showOnActive")
        end,
        hidden = BrAuras.IsRetail(),
        store = true
      },
  ...
  ["Encounter Events"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        values = "difficulty_types",
        test = "%q == BrAuras.InstanceDifficulty()",
        conditionType = "select",
        conditionTest = function(state, needle)
          return BrAuras.InstanceDifficulty() == needle
        end,
        store = true,
        init = "arg"
      },
  ...
  ["Evoker Essence"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local genericShowOn = %s
        local duration, expirationTime, remaining, paused, essence, total = BrAuras.GetEssenceCooldown(triggerEssence)
        local ready = paused
  ...
  ["Death Knight Rune"] = {
    ...
    loadFunc = function(trigger)
      ...
      if (trigger.use_rune) then
        BrAuras.WatchRuneCooldown(trigger.rune);
      else
        for i = 1, 6 do
          BrAuras.WatchRuneCooldown(i);
        end
      end
    end,
    init = function(trigger)
      ...
      if BrAuras.IsWrathOrCataOrMists() then
        ret = [[
          ...
          local startTime, duration = BrAuras.GetRuneCooldown(rune);
        ...
        ret = [[
          ...
          local numRunes = 0;
          for index = 1, 6 do
            local startTime = BrAuras.GetRuneCooldown(index);
            if startTime == 0 then
              numRunes = numRunes + 1;
            end
          end
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        conditionType = "bool",
        enable = function(trigger) return BrAuras.IsWrathOrCataOrMists() and trigger.use_rune end,
        hidden = not BrAuras.IsWrathOrCataOrMists()
      },
      {
        name = "remaining",
        display = L["Remaining Time"],
      ...
      },
      {
        ...
        store = true,
        conditionType = "number",
        enable = function(trigger) return BrAuras.IsWrathOrCataOrMists() and not trigger.use_rune end,
        hidden = not BrAuras.IsWrathOrCataOrMists()
      },
      {
        name = "frostRunes",
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = function(trigger) return BrAuras.IsWrathOrCataOrMists() and trigger.use_bloodRunes or trigger.use_unholyRunes or trigger.use_frostRunes end,
        hidden = not BrAuras.IsWrathOrCataOrMists()
      },
    },
    durationFunc = function(trigger)
      if trigger.use_rune then
        local startTime, duration = BrAuras.GetRuneCooldown(trigger.rune)
  ...
  ["Item Equipped"] = {
    ...
    init = function(trigger)
      ...
      ret = ret ..[[
        ...
        local equipped = BrAuras.CheckForItemEquipped(triggerItemName, itemSlot)
      ]]
    ...
    args = {
      ...
      {
        name = "itemSlot",
        display = BrAuras.newFeatureString .. L["Item Slot"],
        type = "select",
        values = "item_slot_types",
        test = "true",
      },
  ...
  ["Item Bonus Id Equipped"] = {
    ...
    init = function(trigger)
      local ret = [=[
        ...
        local itemBonusId, itemId, itemName, icon, itemSlot, itemSlotString = BrAuras.GetBonusIdInfo(item, useItemSlot and slotSelected)
        local itemBonusId = tonumber(itemBonusId)
        if fetchLegendaryPower then
          itemName, icon = BrAuras.GetLegendaryData(itemBonusId or item)
        end
  ...
  ["Item Set"] = {
    ...
    init = function(trigger)
      local ret = [[
        local setid = %s
        local value, total, name = BrAuras.GetNumSetItemsEquipped(setid)
      ]]
      return string.format(ret, trigger.itemSetId and tonumber(trigger.itemSetId) or "0");
    end,
    GetNameAndIcon = function(trigger)
      local name = select(3, BrAuras.GetNumSetItemsEquipped(trigger.itemSetId and tonumber(trigger.itemSetId) or 0))
      return name, nil
    end,
    statesParameter = "one",
    args = {
      {
        ...
        desc = function()
          ...
          elseif BrAuras.IsClassicEra() then
  ...
  ["Equipment Set"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local itemSetName, icon, numEquipped, numItems = BrAuras.GetEquipmentSetInfo(useItemSetName and triggerItemSetName or nil, partial);
      ]];
    ...
    GetNameAndIcon = function(trigger)
      local name, icon = BrAuras.GetEquipmentSetInfo(trigger.use_itemSetName and trigger.itemSetName or nil, true)
      return name, icon
    end,
    statesParameter = "one",
    args = {
  ...
  ["Threat Situation"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        if unit and unit ~= "none" then
          aggro, status, threatpct, rawthreatpct, threatvalue = BrAuras.UnitDetailedThreatSituation('player', unit)
    ...
    args = {
      ...
      {
        hidden = true,
        test = "BrAuras.UnitExistsFixed(unit, false) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ...
  ["Cast"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local sourceName, sourceRealm = BrAuras.UnitNameWithRealm(unit)
        local destName, destRealm = BrAuras.UnitNameWithRealm(destUnit)
        ...
        spell, _, icon, startTime, endTime, _, _, interruptible, spellId = BrAuras.UnitCastingInfo(unit)
        if spell then
          castType = "cast"
        else
          spell, _, icon, startTime, endTime, _, interruptible, spellId, _, stageTotal = BrAuras.UnitChannelInfo(unit)
          if spell then
            castType = "channel"
    ...
    args = {
      ...
      },
      {
        ...
        enable = BrAuras.IsRetail() and function(trigger) return not trigger.use_inverse end or false,
        hidden = not BrAuras.IsRetail()
      },
      {
        name = "empowered",
      ...
      },
      {
        ...
        display = BrAuras.newFeatureString .. L["Advanced Caster's Target Check"],
      ...
      {
        hidden = true,
        test = "BrAuras.UnitExistsFixed(unit, smart) and ((not inverseTrigger and spell) or (inverseTrigger and not spell)) and specificUnitCheck"
      },
      {
        name = "stagesData",
        hidden = true,
    ...
    overlayFuncs = {
      {
        name = L["Latency"],
        func = function(trigger, state)
          local latency = BrAuras.GetCastLatency()
      ...
      },
      {
        ...
        enable = function(trigger)
          return BrAuras.IsRetail() and trigger.use_showChargedDuration
        end
      }
    },
    GetNameAndIcon = function(trigger)
      ...
      if trigger.use_spellNames and type(trigger.spellNames) == "table" then
        for _, spellName in ipairs(trigger.spellNames) do
          spellId = BrAuras.SafeToNumber(spellName)
          if spellId then
      ...
      if trigger.use_spellIds and type(trigger.spellIds) == "table" then
        for _, spellIdString in ipairs(trigger.spellIds) do
          spellId = BrAuras.SafeToNumber(spellIdString)
          if spellId then
  ...
  ["Character Stats"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = BrAuras.IsWrathOrCataOrMistsOrRetail(),
        conditionType = "number",
        hidden = not BrAuras.IsWrathOrCataOrMistsOrRetail(),
        multiEntry = {
          operator = "and",
          limit = 2
        },
      ...
      },
      {
        ...
        store = true,
        enable = BrAuras.IsWrathOrCataOrMists(),
        conditionType = "number",
        hidden = not BrAuras.IsWrathOrCataOrMists(),
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        display = L["Mastery (%)"],
        type = "number",
        init = "BrAuras.IsCataOrMists() and GetMastery() or GetMasteryEffect()",
        store = true,
        enable = BrAuras.IsCataOrMistsOrRetail(),
        conditionType = "number",
        hidden = not BrAuras.IsCataOrMistsOrRetail(),
      ...
      },
      {
        ...
        type = "number",
        init = "BrAuras.GetEffectiveAttackPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        type = "number",
        init = "BrAuras.GetEffectiveSpellPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
        ...
        enable = BrAuras.IsClassicOrCataOrMists(),
        hidden = not BrAuras.IsClassicOrCataOrMists(),
      },
      {
        type = "header",
      ...
      },
      {
        ...
        type = "number",
        init = BrAuras.IsWrathClassic() and "GetCombatRating(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
      ...
      },
      {
        ...
        type = "number",
        init = BrAuras.IsWrathClassic() and "GetCombatRatingBonus(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
  ...
  ["Conditions"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = BrAuras.IsWrathOrRetail(),
        hidden = not BrAuras.IsWrathOrRetail()
      },
      {
        name = "alive",
        display = L["Alive"],
      ...
      {
        name = "vehicle",
        display = not BrAuras.IsWrathOrCataOrMistsOrRetail() and L["On Taxi"] or L["In Vehicle"],
        type = "tristate",
        init = not BrAuras.IsWrathOrCataOrMistsOrRetail() and "UnitOnTaxi('player')" or "UnitInVehicle('player')",
      },
      {
        name = "resting",
        display = L["Resting"],
      ...
      },
      {
        ...
        type = "multiselect",
        values = "difficulty_types",
        init = "BrAuras.InstanceDifficulty()",
        enable = BrAuras.IsRetail(),
        hidden = not BrAuras.IsRetail(),
      },
      {
  ...
  ["Spell Known"] = {
    type = "spell",
    events = {
      ["events"] = BrAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED","PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED"},
      ["unit_events"] = {
        ["player"] = {"UNIT_PET"}
      }
    },
    ...
    init = function(trigger)
      ...
      if (trigger.use_inverse) then
        table.insert(ret, ([[
          local usePet = %s;
          local active = not spellName or not BrAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      else
        table.insert(ret, ([[
          local usePet = %s;
          local active = spellName and BrAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      end
      return table.concat(ret)
    end,
    ...
    args = {
      ...
      {
        name = "inverse",
        display = BrAuras.newFeatureString .. L["Inverse"],
        type = "toggle",
        test = "true",
      },
      {
  ...
  ["Range Check"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
          local unit = %q;
          local min, max = BrAuras.GetRange(unit, true);
          min = min or 0;
          max = max or 999;
    ...
    args = {
      ...
      },
      {
        ...
        conditionTest = function(state, needle, needle2)
          return state and state.show and BrAuras.CheckRange(state.unit, needle, needle2);
        end,
        noProgressSource = true
      },
      {
  ...
  ["Money"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = BrAuras.newFeatureString..L["Player Money"],
    init = function()
      return [=[
  ...
  ["Currency"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = BrAuras.newFeatureString..L["Currency"],
    triggerFunction = function(trigger)
  ...
  ["Location"] = {
    ...
    name = BrAuras.newFeatureString..L["Location"],
    init = function(trigger)
      local ret = [=[
...
if C_AssistedCombat and C_AssistedCombat.GetNextCastSpell then
  Private.event_prototypes["Assisted Combat Next Cast"] = {
    ...
    loadFunc = function()
      BrAuras.WatchForAssistedCombatNextCast()
    end,
