...
Private.time_format_types = {
  [0] = L["BrAuras Built-In (63:42 | 3:07 | 10 | 2.4)"],
...
elseif BrAuras.IsClassicOrWrathOrCataOrMists() then
  local NUMBER_ABBREVIATION_DATA_FIXED = {
        -- Work around another bug in NUMBER_ABBREVIATION_DATA, https://github.com/BrAuras/BrAuras2/issues/6061
...
local simpleFormatters = {
  ...
  time = {
    ...
    -- Modern Blizzard
    [2] = BrAuras.IsRetail() and function(value)
...
Private.format_types = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      if abbreviate and pad then
        return function(input)
          return BrAuras.PadString(BrAuras.WA_Utf8Sub(input, abbreviateMax), padMode, padLength)
        end
      elseif pad then
        return function(input)
          return BrAuras.PadString(input, padMode, padLength)
        end
      elseif abbreviate then
        return function(input)
          return BrAuras.WA_Utf8Sub(input, abbreviateMax)
        end
      end
      return nil
    end
  ...
  BigNumber = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      if pad then
        return function(input)
          return BrAuras.PadString(formatterFunc(input), padMode, padLength)
        end
      end
    ...
    CreateFormatter = function(symbol, get, withoutColor)
      ...
      if realm == "never" then
        nameFunc = function(unit)
          return unit and BrAuras.UnitName(unit) or ""
        end
      elseif realm == "star" then
        nameFunc = function(unit)
          ...
          local name, realm = BrAuras.UnitName(unit)
          if realm then
            return name .. "*"
          end
          return name or ""
      ...
      elseif realm == "always" then
        nameFunc = function(unit)
          ...
          local name, realm = BrAuras.UnitNameWithRealmCustomName(unit)
          return name .. "-" .. realm
        end
      end
    ...
    CreateFormatter = function(symbol, get, withoutColor)
      ...
      if realm == "never" then
        nameFunc = function(name, realm)
          return BrAuras.GetName(name)
        end
      elseif realm == "star" then
        nameFunc = function(name, realm)
          name = BrAuras.GetName(name)
          if realm ~= "" then
            return name .. "*"
          end
          return name
      ...
      elseif realm == "always" then
        nameFunc = function(name, realm)
          name = BrAuras.GetName(name)
          if realm == "" then
            realm = select(2, BrAuras.UnitNameWithRealm("player"))
          end
          return name .. "-" .. realm
        end
      end
  ...
  GCDTime = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      return function(value, state)
        ...
        local now = GetTime()
        if gcd then
          local gcdDuration, gcdExpirationTime = BrAuras.GetGCDInfo()
        ...
        if cast then
          local _, _, _, _, endTime = BrAuras.UnitCastingInfo("player")
        ...
        if channel then
          local _, _, _, _, endTime = BrAuras.UnitChannelInfo("player")
          local castExpirationTime = endTime and endTime > 0 and (endTime / 1000) or 0
          if castExpirationTime > 0 then
            result = min(result, now + value - castExpirationTime)
          end
        ...
        return numberToStringFunc(result / BrAuras.CalculatedGcdDuration())
      end
    end
  }
}
...
if not BrAuras.IsClassicEra() then
...
for classID = 1, 20 do -- 20 is for GetNumClasses() but that function doesn't exists on Classic
  ...
  if classInfo and classID ~=14 then -- 14 == Adventurer
    BrAuras.class_types[classInfo.classFile] = WrapTextInColorCode(classInfo.className, WA_GetClassColor(classInfo.classFile))
  end
end
...
do
  local races = {
    ...
    [7] = true,
    [8] = true,
    [9] = not BrAuras.IsClassicOrWrath() and true or nil, -- Goblin
    [10] = true,
    [11] = true,
    [22] = true,
    [24] = true,
  ...
  for raceId, enabled in pairs(races) do
    ...
    if raceInfo then
      BrAuras.race_types[raceInfo.clientFileString] = raceInfo.raceName
    end
  end
end
...
BrAuras.spec_types_specific = {}

---@type table<number, string>
Private.spec_types_all = {}
Private.specs_sorted = {}
local function update_specs()
  for _, classFileName in pairs(BrAuras.classes_sorted) do
    local classID = BrAuras.class_ids[classFileName]
    BrAuras.spec_types_specific[classFileName] = {}
    local numSpecs = BrAuras.IsCataClassic() and 3 or Private.ExecEnv.GetNumSpecializationsForClassID(classID) -- see https://github.com/Stanzilla/WoWUIBugs/issues/559
    for i = 1, numSpecs do
      local specId, tabName, _, icon = Private.ExecEnv.GetSpecializationInfoForClassID(classID, i);
      if tabName then
        tinsert(BrAuras.spec_types_specific[classFileName], "|T"..(icon or "error")..":0|t "..(tabName or "error"));
...
Private.texture_types = {
  ...
  ["Icons"] = {
    ...
    ["240972"] = "Poison Skull",
    ["241049"] = "Star",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\cancel-icon.tga"] = "Cancel Icon",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\cancel-mark.tga"] = "Cancel Mark",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\emoji.tga"] = "Emoji",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\exclamation-mark.tga"] = "Exclamation Mark",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\eyes.tga"] = "Eyes",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\ok-icon.tga"] = "Ok Icon",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\targeting-mark.tga"] = "Targeting Mark",
  ...
  ["Beams"] = {
    ...
    ["369750"] = "Shadow Beam",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\rainbowbar"] = "Rainbow Bar",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\StripedTexture"] = "Striped Bar",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\stripe-bar.tga"] = "Striped Bar 2",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\stripe-rainbow-bar.tga"] = "Rainbow Bar 2",
  },
  ["Shapes"] = {
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_Smooth"] = "Smooth Circle",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_Smooth_Border"] = "Smooth Circle with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_Squirrel"] = "Spiralled Circle",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_Squirrel_Border"] = "Spiralled Circle with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_White"] = "Circle",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_White_Border"] = "Circle with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_Smooth"] = "Smooth Square",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_Smooth_Border"] = "Smooth Square with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_Smooth_Border2"] = "Smooth Square with Border 2",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_Squirrel"] = "Spiralled Square",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_Squirrel_Border"] = "Spiralled Square with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_White"] = "Square",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_White_Border"] = "Square with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_FullWhite"] = "Full White Square",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Triangle45"] = "45° Triangle",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Trapezoid"] = "Trapezoid",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\triangle-border.tga"] = "Triangle with Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\triangle.tga"] = "Triangle",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_Smooth2.tga"] = "Smooth Circle Small",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\circle_border5.tga"] = "Circle Border",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\ring_glow3.tga"] = "Circle Border Glow",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\square_mini.tga"] = "Small Square",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\target_indicator.tga"] = "Target Indicator",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\target_indicator_glow.tga"] = "Target Indicator Glow",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\arrows_target.tga"] = "Arrows Target",

    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_AlphaGradient_In.tga"] = "Circle Alpha Gradient In",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Circle_AlphaGradient_Out.tga"] = "Circle Alpha Gradient Out",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Ring_10px.tga"] = "Ring 10px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Ring_20px.tga"] = "Ring 20px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Ring_30px.tga"] = "Ring 30px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Ring_40px.tga"] = "Ring 40px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_AlphaGradient.tga"] = "Square Alpha Gradient",

    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\square_border_1px.tga"] = "Square Border 1px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\square_border_5px.tga"] = "Square Border 5px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\square_border_10px.tga"] = "Square Border 10px",
  },
  ["Sparks"] = {
  ...
  ["BrAuras"] = {
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\logo_64.tga"] = "BrAuras logo 64px",
    ["Interface\\AddOns\\BrAuras\\Media\\Textures\\logo_256.tga"] = "BrAuras logo 256px"
  }
}

if C_Texture and C_Texture.GetAtlasElements then
  if BrAuras.buildType == "dev" and Private.AtlasList then
    BrAuras.prettyPrint("Private.AtlasList can be removed now.")
  end
...
if BrAuras.IsClassicEra() then -- Classic
...
local PowerAurasPath = "Interface\\Addons\\BrAuras\\PowerAurasMedia\\Auras\\"
...
Private.texture_types["PowerAuras Words"] = {
  ...
  [PowerAurasPath.."Aura44"] = "Buff",
  ["Interface\\AddOns\\BrAuras\\Media\\Textures\\interrupt"] = "Interrupt"
}
...
if not BrAuras.IsClassicEra() then
  ...
  local instance_difficulty_names = {
    ...
    [236] = L["Lorewalking"],
    [237] = BrAuras.IsMists() and L["Dungeon (Celestial)"] or unused,
  }
  ...
  for i = 1, 240 do
    ...
    if name then
      if instance_difficulty_names[i] then
        ...
        BrAuras.prettyPrint(string.format("Unknown difficulty id found. You are probably running an outdated version. Debug Information: %s %s %s", i, name, type))
      end
    end
  end
end
...
if BrAuras.IsWrathOrMistsOrRetail() then
  ---@type table<number, string>
...
-- register sounds
LSM:Register("sound", "Heartbeat Single", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\HeartbeatSingle.ogg")
LSM:Register("sound", "Batman Punch", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\BatmanPunch.ogg")
LSM:Register("sound", "Bike Horn", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\BikeHorn.ogg")
LSM:Register("sound", "Boxing Arena Gong", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\BoxingArenaSound.ogg")
LSM:Register("sound", "Bleat", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Bleat.ogg")
LSM:Register("sound", "Cartoon Hop", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\CartoonHop.ogg")
LSM:Register("sound", "Cat Meow", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\CatMeow2.ogg")
LSM:Register("sound", "Kitten Meow", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\KittenMeow.ogg")
LSM:Register("sound", "Robot Blip", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\RobotBlip.ogg")
LSM:Register("sound", "Sharp Punch", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\SharpPunch.ogg")
LSM:Register("sound", "Water Drop", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\WaterDrop.ogg")
LSM:Register("sound", "Air Horn", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\AirHorn.ogg")
LSM:Register("sound", "Applause", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Applause.ogg")
LSM:Register("sound", "Banana Peel Slip", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\BananaPeelSlip.ogg")
LSM:Register("sound", "Blast", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Blast.ogg")
LSM:Register("sound", "Cartoon Voice Baritone", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\CartoonVoiceBaritone.ogg")
LSM:Register("sound", "Cartoon Walking", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\CartoonWalking.ogg")
LSM:Register("sound", "Cow Mooing", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\CowMooing.ogg")
LSM:Register("sound", "Ringing Phone", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\RingingPhone.ogg")
LSM:Register("sound", "Roaring Lion", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\RoaringLion.ogg")
LSM:Register("sound", "Shotgun", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Shotgun.ogg")
LSM:Register("sound", "Squish Fart", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\SquishFart.ogg")
LSM:Register("sound", "Temple Bell", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\TempleBellHuge.ogg")
LSM:Register("sound", "Torch", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Torch.ogg")
LSM:Register("sound", "Warning Siren", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\WarningSiren.ogg")
...
LSM:Register("sound", "Sheep Blerping", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\SheepBleat.ogg")
LSM:Register("sound", "Rooster Chicken Call", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\RoosterChickenCalls.ogg")
LSM:Register("sound", "Goat Bleeting", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\GoatBleating.ogg")
LSM:Register("sound", "Acoustic Guitar", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\AcousticGuitar.ogg")
LSM:Register("sound", "Synth Chord", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\SynthChord.ogg")
LSM:Register("sound", "Chicken Alarm", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\ChickenAlarm.ogg")
LSM:Register("sound", "Xylophone", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Xylophone.ogg")
LSM:Register("sound", "Drums", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Drums.ogg")
LSM:Register("sound", "Tada Fanfare", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\TadaFanfare.ogg")
LSM:Register("sound", "Squeaky Toy Short", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\SqueakyToyShort.ogg")
LSM:Register("sound", "Error Beep", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\ErrorBeep.ogg")
LSM:Register("sound", "Oh No", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\OhNo.ogg")
LSM:Register("sound", "Double Whoosh", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\DoubleWhoosh.ogg")
LSM:Register("sound", "Brass", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Brass.mp3")
LSM:Register("sound", "Glass", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Glass.mp3")

LSM:Register("sound", "Voice: Adds", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Adds.ogg")
LSM:Register("sound", "Voice: Boss", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Boss.ogg")
LSM:Register("sound", "Voice: Circle", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Circle.ogg")
LSM:Register("sound", "Voice: Cross", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Cross.ogg")
LSM:Register("sound", "Voice: Diamond", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Diamond.ogg")
LSM:Register("sound", "Voice: Don't Release", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\DontRelease.ogg")
LSM:Register("sound", "Voice: Empowered", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Empowered.ogg")
LSM:Register("sound", "Voice: Focus", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Focus.ogg")
LSM:Register("sound", "Voice: Idiot", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Idiot.ogg")
LSM:Register("sound", "Voice: Left", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Left.ogg")
LSM:Register("sound", "Voice: Moon", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Moon.ogg")
LSM:Register("sound", "Voice: Next", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Next.ogg")
LSM:Register("sound", "Voice: Portal", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Portal.ogg")
LSM:Register("sound", "Voice: Protected", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Protected.ogg")
LSM:Register("sound", "Voice: Release", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Release.ogg")
LSM:Register("sound", "Voice: Right", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Right.ogg")
LSM:Register("sound", "Voice: Run Away", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\RunAway.ogg")
LSM:Register("sound", "Voice: Skull", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Skull.ogg")
LSM:Register("sound", "Voice: Spread", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Spread.ogg")
LSM:Register("sound", "Voice: Square", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Square.ogg")
LSM:Register("sound", "Voice: Stack", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Stack.ogg")
LSM:Register("sound", "Voice: Star", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Star.ogg")
LSM:Register("sound", "Voice: Switch", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Switch.ogg")
LSM:Register("sound", "Voice: Taunt", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Taunt.ogg")
LSM:Register("sound", "Voice: Triangle", "Interface\\AddOns\\BrAuras\\Media\\Sounds\\Triangle.ogg")

local PowerAurasSoundPath = "Interface\\Addons\\BrAuras\\PowerAurasMedia\\Sounds\\"
...
LSM.RegisterCallback(BrAuras, "LibSharedMedia_Registered", function(_, mediatype, key)
...
LSM:Register("font", "Fira Mono Medium", "Interface\\Addons\\BrAuras\\Media\\Fonts\\FiraMono-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
-- Other Fira fonts
LSM:Register("font", "Fira Sans Black", "Interface\\Addons\\BrAuras\\Media\\Fonts\\FiraSans-Heavy.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Condensed Black", "Interface\\Addons\\BrAuras\\Media\\Fonts\\FiraSansCondensed-Heavy.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Condensed Medium", "Interface\\Addons\\BrAuras\\Media\\Fonts\\FiraSansCondensed-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Medium", "Interface\\Addons\\BrAuras\\Media\\Fonts\\FiraSans-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "PT Sans Narrow Regular", "Interface\\Addons\\BrAuras\\Media\\Fonts\\PTSansNarrow-Regular.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "PT Sans Narrow Bold", "Interface\\Addons\\BrAuras\\Media\\Fonts\\PTSansNarrow-Bold.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)

-- register plain white border
LSM:Register("border", "Square Full White", [[Interface\AddOns\BrAuras\Media\Textures\Square_FullWhite.tga]])

LSM:Register("statusbar", "Clean", [[Interface\AddOns\BrAuras\Media\Textures\Statusbar_Clean]])
LSM:Register("statusbar", "Stripes", [[Interface\AddOns\BrAuras\Media\Textures\Statusbar_Stripes]])
LSM:Register("statusbar", "Thick Stripes", [[Interface\AddOns\BrAuras\Media\Textures\Statusbar_Stripes_Thick]])
LSM:Register("statusbar", "Thin Stripes", [[Interface\AddOns\BrAuras\Media\Textures\Statusbar_Stripes_Thin]])
LSM:Register("border", "Drop Shadow", [[Interface\AddOns\BrAuras\Media\Textures\Border_DropShadow]])
...
Private.author_option_fields = {
  ...
  media = {
    mediaType = "sound",
    media = "Interface\\AddOns\\BrAuras\\Media\\Sounds\\AirHorn.ogg"
  },
  multiselect = {
    default = {true},
    values = {"val1"},
...
Private.author_option_media_defaults = {
  sound = "Interface\\AddOns\\BrAuras\\Media\\Sounds\\AirHorn.ogg",
...
BrAuras.item_weapon_types = Private.item_weapon_types

BrAuras.StopMotion = BrAuras.StopMotion or {}
BrAuras.StopMotion.texture_types = BrAuras.StopMotion.texture_types or {}
BrAuras.StopMotion.texture_data = BrAuras.StopMotion.texture_data or {}

BrAuras.StopMotion.texture_types.Basic = {
  ["Interface\\AddOns\\BrAuras\\Media\\Textures\\stopmotion"] = "Example",
}

BrAuras.StopMotion.texture_data["Interface\\AddOns\\BrAuras\\Media\\Textures\\stopmotion"] = { count = 64, rows = 8, columns = 8 }


BrAuras.StopMotion.animation_types = {
  loop = L["Loop"],
