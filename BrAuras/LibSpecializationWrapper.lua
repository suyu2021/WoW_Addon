...
if not BrAuras.IsCataOrMistsOrRetail() then return end
---@type string
local AddonName = ...
---@class Private
...
if LibSpec then
  ...
  frame:SetScript("OnEvent", function()
    ...
    if IsInRaid() then
      ...
      for i = 1, max do
        local name = GetUnitName(BrAuras.raidUnits[i], true)
        nameToUnitMap[name] = BrAuras.raidUnits[i]
      end
    else
      local max = GetNumSubgroupMembers()
      for i = 1, max do
        local name = GetUnitName(BrAuras.partyUnits[i], true)
        nameToUnitMap[name] = BrAuras.partyUnits[i]
      end
    end
  ...
  if BrAuras.IsRetail() then
    function Private.LibSpecWrapper.CheckTalentForUnit(unit, talentId)
      if UnitIsUnit(unit, "player") then
        return select(4, BrAuras.GetTalentById(talentId))
      end
...
BrAuras.SpecForUnit = Private.LibSpecWrapper.SpecForUnit
BrAuras.SpecRolePositionForUnit = Private.LibSpecWrapper.SpecRolePositionForUnit
BrAuras.CheckTalentForUnit = Private.LibSpecWrapper.CheckTalentForUnit

if Private.LibSpecWrapper.CheckGlyphForUnit then
  BrAuras.CheckGlyphForUnit = Private.LibSpecWrapper.CheckGlyphForUnit
else
  BrAuras.CheckGlyphForUnit = function() return nil end
end
