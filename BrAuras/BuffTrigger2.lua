...
local function UnitInSubgroupOrPlayer(unit, includePets)
  ...
  elseif includePets == "PlayersAndPets" then
    return UnitInSubgroup(BrAuras.petUnitToUnit[unit] or unit) or UnitIsUnit("player", unit) or UnitIsUnit("pet", unit)
  elseif includePets == "PetsOnly" then
    return UnitInSubgroup(BrAuras.petUnitToUnit[unit]) or UnitIsUnit("pet", unit)
  end
end
...
local function UpdateToolTipDataInMatchData(matchData, time)
  ...
  if matchData.unit and matchData.auraInstanceID then
    local dataInstanceID, tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = BrAuras.GetAuraInstanceTooltipInfo(matchData.unit, matchData.auraInstanceID, matchData.filter)
  ...
  elseif matchData.unit and matchData.index and matchData.filter then
    local tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = BrAuras.GetAuraTooltipInfo(matchData.unit, matchData.index, matchData.filter)
    changed = matchData.tooltip ~= tooltip or matchData.tooltip1 ~= tooltip1
      or matchData.tooltip2 ~= tooltip2 or matchData.tooltip3 ~= tooltip3 or matchData.tooltip4 ~= tooltip4
    matchData.tooltip, matchData.tooltip1, matchData.tooltip2, matchData.tooltip3, matchData.tooltip4 = tooltip, tooltip1, tooltip2, tooltip3, tooltip4
  end
...
local function UpdateMatchData(time, matchDataChanged, unit, index, auraInstanceID, filter, name, icon, stacks, debuffClass, duration, expirationTime, unitCaster, isStealable, isBossDebuff, isCastByPlayer, spellId, modRate, points)
  ...
  local debuffClassIcon = BrAuras.EJIcons[debuffClass]
...
local function UpdateStateWithMatch(time, bestMatch, triggerStates, cloneId, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local debuffClassIcon = BrAuras.EJIcons[bestMatch.debuffClass]
...
local function UpdateStateWithNoMatch(time, triggerStates, triggerInfo, cloneId, unit, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local fallbackName, fallbackIcon = BuffTrigger.GetNameAndIconSimple(BrAuras.GetData(triggerInfo.id), triggerInfo.triggernum)
  if not triggerStates[cloneId] then
    triggerStates[cloneId] = {
      show = true,
      changed = true,
    ...
    if allUnits then
      ...
      return function()
        if raid then
          if i <= 40 then
            ...
            if includePets == "PlayersAndPets" then
              ret = pets and BrAuras.raidpetUnits[i] or BrAuras.raidUnits[i]
              pets = not pets
              if pets then
                i = i + 1
              end
            elseif includePets == "PetsOnly" then
              ret = BrAuras.raidpetUnits[i]
              i = i + 1
            else -- raid
              ret = BrAuras.raidUnits[i]
              i = i + 1
            end
            return ret
          end
        ...
        if i <= 4 then
          local ret
          if includePets == "PlayersAndPets" then
            ret = pets and BrAuras.partypetUnits[i] or BrAuras.partyUnits[i]
            pets = not pets
            if pets then
              i = i + 1
            end
          elseif includePets == "PetsOnly" then
            ret = BrAuras.partypetUnits[i]
            i = i + 1
          else -- group
            ret = BrAuras.partyUnits[i]
            i = i + 1
          end
          return ret
        end
...
local function TriggerInfoApplies(triggerInfo, unit)
  ...
  if BrAuras.UnitIsPet(unit) then
    controllingUnit = BrAuras.petUnitToUnit[unit]
  end
  ...
  if triggerInfo.raidRole and not triggerInfo.raidRole[BrAuras.UnitRaidRole(controllingUnit) or ""] then
    return false
  end
  ...
  if triggerInfo.hostility and BrAuras.GetPlayerReaction(unit) ~= triggerInfo.hostility then
    return false
  end
  ...
  if triggerInfo.nameChecker and not triggerInfo.nameChecker:Check(BrAuras.UnitNameWithRealm(unit)) then
    return false
  end
...
Private.frames["BrAuras Buff2 Frame"] = Buff2Frame
...
local function EventHandler(frame, event, arg1, arg2, ...)
  ...
  elseif event == "UNIT_PET" then
    local pet = BrAuras.unitToPetUnit[arg1]
    if pet then
...
if BrAuras.IsCataOrMistsOrRetail() then
  Private.LibSpecWrapper.Register(function(unit)
    ...
    local deactivatedTriggerInfos = {}
    RecheckActiveForUnitType("group", unit, deactivatedTriggerInfos)
    RecheckActiveForUnitType("group", BrAuras.unitToPetUnit[unit], deactivatedTriggerInfos)
    DeactivateScanFuncs(deactivatedTriggerInfos)
...
local function createScanFunc(trigger)
  ...
  if use_ignore_name then
    ...
    for index, spellName in ipairs(trigger.ignoreAuraNames) do
      local spellId = BrAuras.SafeToNumber(spellName)
  ...
  if use_ignore_spellId then
    ...
    for index, spellId in ipairs(trigger.ignoreAuraSpellids) do
      local spell = BrAuras.SafeToNumber(spellId)
      if spell then
  ...
  return {
    untrackableSoftTarget = {
      ...
      check = function(trigger)
        return BrAuras.IsUntrackableSoftTarget(trigger.unit)
      end
    }
  }
end
...
function BuffTrigger.Add(data)
  ...
  for triggernum, triggerData in ipairs(data.triggers) do
    ...
    if trigger.type == "aura2" then
      ...
      local effectiveGroupRole = BrAuras.IsWrathOrCataOrMistsOrRetail() and (groupTrigger and trigger.useGroupRole and trigger.group_role) or nil
      local effectiveRaidRole = BrAuras.IsClassicOrWrathOrCataOrMists() and (groupTrigger and trigger.useRaidRole and trigger.raid_role) or nil
      local effectiveClass = groupTrigger and trigger.useClass and trigger.class
      local effectiveSpecId = BrAuras.IsCataOrMistsOrRetail() and (groupTrigger and trigger.useActualSpec and trigger.actualSpec) or nil
      local effectiveArenaSpec = BrAuras.IsRetail() and (trigger.unit == "arena" and trigger.useArenaSpec and trigger.arena_spec) or nil
      ...
      local effectiveInRange = BrAuras.IsRetail() and groupTrigger and trigger.inRange
      ...
      local triggerInformation = {
        ...
        fetchRole = BrAuras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole,
...
function BuffTrigger.GetNameAndIcon(data, triggernum)
  ...
  if (not name or not icon and BrAuras.spellCache) then
    local trigger = data.triggers[triggernum].trigger
    if trigger.useName and trigger.auranames then
      for index, spellName in ipairs(trigger.auranames) do
        icon = BrAuras.spellCache.GetIcon(spellName)
        if icon then
...
function BuffTrigger.GetAdditionalProperties(data, triggernum)
  ...
  if BrAuras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole then
...
function BuffTrigger.GetTriggerConditions(data, triggernum)
  ...
  result["nameCaster"] = {
    ...
    test = function(state, needle, op, preamble)
      return state.unitCaster and preamble:Check(BrAuras.UnitNameWithRealm(state.unitCaster))
    end,
    operator_types = "none",
  }
...
function BuffTrigger.GetTriggerDescription(data, triggernum, namestable)
  ...
  if trigger.useName and trigger.auranames then
    for index, name in pairs(trigger.auranames) do
      ...
      local icon
      local spellId = BrAuras.SafeToNumber(name)
      if spellId then
        icon = Private.ExecEnv.GetSpellIcon(spellId)
      else
        icon = BrAuras.spellCache.GetIcon(name)
      end
...
BrAuras.RegisterTriggerSystem({"aura2"}, BuffTrigger)
