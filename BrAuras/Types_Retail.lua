...
local classIDToOffsets = {
	[1] = { extraOffsetX = BrAuras.IsTWW() and 60 or 30, extraOffsetY = 31, }, -- Warrior
...
function Private.GetTalentData(specId)
  ...
  local specDataByNodeId = {}
	...
	local subTreeIDs = BrAuras.IsTWW() and C_ClassTalents.GetHeroTalentSpecsForClassSpec(configId, specId) or {}
...
for k, v in pairs(texture_data) do
	...
	if atlasInfo then
		...
		BrAuras.StopMotion.texture_data[k] = v
		BrAuras.StopMotion.texture_types.Blizzard[k] = k
	end
end
