...
local WA_ClassColorName = function(unit, maxlen)
  if unit and UnitExists(unit) then
    local name = BrAuras.UnitName(unit)
...
BrAuras.WA_ClassColorName = WA_ClassColorName

BrAuras.PadString = function(input, padMode, padLength)
  input = tostring(input)
  if type(input) ~= "string" then
    return input
  end
...
BrAuras.ShowOverlayGlow = LCG.ButtonGlow_Start
BrAuras.HideOverlayGlow = LCG.ButtonGlow_Stop

local LGF = LibStub("LibGetFrame-1.0")
BrAuras.GetUnitFrame = LGF.GetUnitFrame
BrAuras.GetUnitNameplate =  function(unit)
...
-- Stack of of aura environments/uids, allows use of recursive aura activations through calls to BrAuras.ScanEvents().
...
function Private.ActivateAuraEnvironment(id, cloneId, state, states, onlyConfig)
  local data = id and BrAuras.GetData(id)
  if not data then
    ...
    if environment_initialized[id] == 2 or (onlyConfig and environment_initialized[id] == 1) then
      local region = BrAuras.GetRegion(id, cloneId)
...
local FakeBrAurasMixin = {
  blockedFunctions = {
    ...
    ShowOptions = true,
    -- Note these shouldn't exist in the BrAuras namespace, but moving them takes a bit of effort,
  ...
  blockedTables = {
    ...
    -- checked for BrAuras.loaded in there
    loaded = true
  },
  override = {
    ...
    GetData = function(id)
      ...
      if getDataCallCounts[currentId] > 99 then
        Private.AuraWarnings.UpdateWarning(current_uid, "FakeBrAurasGetData", "warning",
    ...
    clones = MakeDeprecated(Private.clones, "clones",
                L["Using BrAuras.clones is deprecated. Use BrAuras.GetRegion(id, cloneId) instead."]),
    regions = MakeDeprecated(Private.regions, "regions",
                L["Using BrAuras.regions is deprecated. Use BrAuras.GetRegion(id) instead."]),
  ...
  blocked = blocked,
  setBlocked = function()
    Private.AuraWarnings.UpdateWarning(current_uid, "FakeBrAurasSet", "error",
                  L["Writing to the BrAuras table is not allowed."], true)
  end
}

local FakeBrAuras = MakeReadOnly(BrAuras, FakeBrAurasMixin)
...
local overridden = {
  ...
  ActionButton_ShowOverlayGlow = BrAuras.ShowOverlayGlow,
  ActionButton_HideOverlayGlow = BrAuras.HideOverlayGlow,
  BrAuras = FakeBrAuras
}
...
function BrAuras.LoadFunction(string, id)
