...
Enable this setting if you want this timer to be hidden, or when using a BrAuras text to display the timer]=] ] = [=[Отсчет времени будет отображаться в соответствии с настройками интерфейса (переопределено некоторыми аддонами).

Включите этот параметр, если вы хотите скрыть этот отсчет или использовать текст BrAuras для его отображения.]=]
...
Например: party4, raid7, arena3, boss2, nameplate6, target, focus, pet и др.]=]
	...
	L["Chat with BrAuras experts on our Discord server."] = "Общайтесь со знатоками BrAuras на нашем сервере Discord."
...
Поскольку это триггер статуса, указанные события могут быть переданы BrAuras без ожидаемых аргументов.
...
|cFF00CC00> 1/2|r - сработает, если задействовано больше половины единиц (5 из 5, 6 из 10)]=]
	...
	L["ModelPaths could not be loaded, the addon is %s"] = "Не удалось загрузить BrAuras Model Paths. Причина - %s"
...
every 3 events starting from 2nd and ending at 11th: 2-11/3]=] ] = "Время наступления события, сбрасываемое при выгрузке ауры. Может быть диапазоном значений. Может содержать несколько значений, разделённых запятой или пробелом. Примеры: 2-е, 5-е и 6-е события: 2, 5, 6; 2-е по 6-е: 2-6; каждые 2 события: /2; каждые 3 события, начиная с 2-го: 2/3; каждые 3 события, начиная с 2-го и заканчивая 11-м: 2-11/3."
	...
	L["Requires LibSpecialization, that is e.g. a up-to date BrAuras version"] = "Требуется LibSpecialization, т.е. актуальная версия BrAuras"
...
(исходного положения)]=]
	...
	L["Templates could not be loaded, the addon is %s"] = "Не удалось загрузить BrAuras Templates. Причина - %s"
...
Конечная анимация не начнет отображаться, пока индикация не будет нормально скрыта  (должен сработать детриггер).]=]
	...
	L["The BrAuras Options Addon version %s doesn't match the BrAuras version %s. If you updated the addon while the game was running, try restarting World of Warcraft. Otherwise try reinstalling BrAuras"] = "Версия %s дополнения BrAuras Options не соответствует версии %s BrAuras. Если вы обновили дополнение во время игры, попробуйте перезапустить World of Warcraft. В противном случае попробуйте переустановить BrAuras"
	L["Then "] = "Тогда "
...
Upgrade your version of BrAuras or wait for next release before installing this aura.]=] ] = [=[Индикация была создана в новой версии BrAuras.
Прежде чем импортировать её, обновите вашу версию аддона или дождитесь следующего релиза BrAuras.]=]
	...
	L["BrAuras %s on WoW %s"] = "BrAuras: %s. Интерфейс: %s"
	--[[Translation missing --]]
...
BrAuras will always run custom anchor code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "Вы можете добавить сюда список значений состояния, разделенных запятыми, при изменении которых BrAuras также выполнит код привязки. BrAuras всегда выполнит пользовательский код привязки, если вы включите 'изменен' в этот список, или когда регион будет добавлен, удален или переупорядочен."
	L[ [=[You can add a comma-separated list of state values here that (when changed) BrAuras should also run the Grow Code on.

BrAuras will always run custom grow code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "Вы можете добавить сюда список значений состояния, разделенных запятыми, при изменении которых BrAuras также выполнит код увеличения. BrAuras всегда выполнит пользовательский код увеличения, если вы включите 'изменен' в этот список, или когда регион будет добавлен, удален или переупорядочен."
	L["You can add a comma-separated list of state values here that (when changed) BrAuras should also run the sort code on.BrAuras will always run custom sort code if you include 'changed' in this list, or when a region is added, removed."] = "Вы можете добавить сюда список значений состояния, разделенных запятыми, при изменении которых BrAuras также выполнит код сортировки. BrAuras всегда выполнит пользовательский код сортировки, если вы включите 'изменен' в этот список, или когда регион будет добавлен или удален."
