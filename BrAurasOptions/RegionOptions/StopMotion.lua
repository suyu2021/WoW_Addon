...
local texture_types = BrAuras.StopMotion.texture_types;
local texture_data = BrAuras.StopMotion.texture_data;
local animation_types = BrAuras.StopMotion.animation_types;
...
local function modifyThumbnail(parent, region, data, fullModify, size)
    ...
    local texture = data.foregroundTexture or "Interface\\AddOns\\BrAuras\\Media\\Textures\\stopmotion";
    ...
    if (region.foreground.rows and region.foreground.columns) then
      ...
      BrAuras.setTile(region.texture, frame, region.foreground.rows, region.foreground.columns, frameScaleW, frameScaleH);

      region.SetValue = function(self, percent)
        local frame = floor(percent * (region.endFrame - region.startFrame) + region.startFrame);
        BrAuras.setTile(self.texture, frame, region.foreground.rows, region.foreground.columns, frameScaleW, frameScaleH);
      end
    else
...
local function createIcon()
    local data = {
        height = 30,
        width = 30,
        foregroundTexture = "Interface\\AddOns\\BrAuras\\Media\\Textures\\stopmotion",
