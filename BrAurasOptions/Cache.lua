...
function spellCache.Build()
  if not cache  then
    error("spellCache has not been loaded. Call BrAuras.spellCache.Load(...) first.")
  end
...
function spellCache.Load(data)
  ...
  local version = BrAuras.versionString
...
function spellCache.CorrectAuraName(input)
  ...
  local spellId = BrAuras.SafeToNumber(input)
  if type(input) == "string" and input:find("|", nil, true) then
    spellId = BrAuras.SafeToNumber(input:match("|Hspell:(%d+)"))
  end
  if(spellId) then
