...
local Type, Version = "BrAurasDisplayButton", 60
  ...
  func = function()
    if (not IsRegionAGroup(clipboard.source) and IsRegionAGroup(clipboard.current)) then
      ...
      for child in OptionsPrivate.Private.TraverseLeafs(clipboard.current) do
        ...
        BrAuras.Add(child)
        BrAuras.ClearAndUpdateOptions(child.id)
      end
    else
      copyAuraPart(clipboard.source, clipboard.current, clipboard.part);
      BrAuras.Add(clipboard.current)
      BrAuras.ClearAndUpdateOptions(clipboard.current.id)
    end

    BrAuras.FillOptions()
    OptionsPrivate.Private.ScanForLoads({[clipboard.current.id] = true});
    OptionsPrivate.SortDisplayButtons(nil, true);
    BrAuras.PickDisplay(clipboard.current.id);
    BrAuras.UpdateThumbnail(clipboard.current.id);
    BrAuras.ClearAndUpdateOptions(clipboard.current.id);
  end
}
...
local Actions = {
    ...
    if source and not source.data.parent then
      if groupId then
        ...
        if group and group:IsGroup() then
          ...
          BrAuras.Add(source.data)
          BrAuras.Add(group.data)
          OptionsPrivate.Private.AddParents(group.data)
          BrAuras.UpdateGroupOrders(group.data)
          BrAuras.ClearAndUpdateOptions(group.data.id)
          BrAuras.ClearAndUpdateOptions(source.data.id)
  ...
  ["Ungroup"] =  function(source)
    if source and source.data.parent then
      local parent = BrAuras.GetData(source.data.parent)
      ...
      if ensure(children, index, source.data.id) then
        ...
        BrAuras.UpdateGroupOrders(parent);
        BrAuras.ClearAndUpdateOptions(parent.id);
...
local function IsParentRecursive(needle, parent)
  ...
  if needle.parent then
    local needleParent = BrAuras.GetData(needle.parent)
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      elseif(IsShiftKeyDown()) then
        ...
        if(editbox) then
          ...
          if not (GetCurrentRegion() == 5 or GetLocale() == "zhCN") then -- China region (5), and chinese locale profanity filter doesn't allow links in chat
            ...
            editbox:Insert("[BrAuras: "..fullName.." - "..self.data.id.."]"..url)
          else
            editbox:Insert("[BrAuras: "..fullName.." - "..self.data.id.."]")
          end
        ...
        if(mouseButton == "RightButton") then
          ...
          if(OptionsPrivate.IsDisplayPicked(self.data.id) and OptionsPrivate.IsPickedMultiple()) then
            LibDD:EasyMenu(OptionsPrivate.MultipleDisplayTooltipMenu(), BrAuras_DropDownMenu, self.frame, 0, 0, "MENU");
          else
            UpdateClipboardMenuEntry(self.data);
            LibDD:EasyMenu(self.menu, BrAuras_DropDownMenu, self.frame, 0, 0, "MENU");
            if not(OptionsPrivate.IsDisplayPicked(self.data.id)) then
              if self.data.controlledChildren then
                BrAuras.PickDisplay(self.data.id, "group")
              else
                BrAuras.PickDisplay(self.data.id);
              end
            end
          end
        else
    ...
    function self.callbacks.OnClickGrouping()
      if (BrAuras.IsImporting()) then return end;
      for index, selectedId in ipairs(self.grouping) do
        local selectedData = BrAuras.GetData(selectedId);
      ...
      BrAuras.FillOptions();
      BrAuras.UpdateGroupOrders(self.data);
    ...
    function self.callbacks.OnUpGroupClick()
      ...
      if(self.data.parent) then
        ...
        local parentData = BrAuras.GetData(self.data.parent);
        local index;
        ...
        if(index) then
          if(index <= 1) then
            ...
            OptionsPrivate.Private.Animate("button", BrAuras.GetData(parentData.controlledChildren[index-1]).uid, "main", updata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", BrAuras.GetData(parentData.controlledChildren[index]).uid, "main", downdata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            BrAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
      ...
      if(self.data.parent) then
        ...
        if(index) then
          if(index >= #parentData.controlledChildren) then
            ...
            local updata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = -32};
            local downdata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = 32};
            OptionsPrivate.Private.Animate("button", BrAuras.GetData(parentData.controlledChildren[index+1]).uid, "main", downdata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", BrAuras.GetData(parentData.controlledChildren[index]).uid, "main", updata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            BrAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
    ...
    function self.callbacks.OnRenameAction(newid)
      ...
      if not(newid == oldid) then
        BrAuras.Rename(self.data, newid);
      end
      self:UpdateParentWarning()
    end
  ...
  ["Ungroup"] = function(self)
    ...
    local newParent = parentData.parent and BrAuras.GetData(parentData.parent)
    if newParent then
    ...
    self:UpdateIconsVisible()
    if newParent then
      BrAuras.Add(newParent)
      OptionsPrivate.Private.AddParents(newParent)
      BrAuras.ClearAndUpdateOptions(newParent.id)
      BrAuras.UpdateGroupOrders(newParent)
    end
    BrAuras.ClearAndUpdateOptions(self.data.id);
    ...
    if picked then
      ...
      if mode == "GROUP" then
        if self.data.regionType == "dynamicgroup" then
          ...
          local parentData = self.data.parent and BrAuras.GetData(self.data.parent)
  ...
  ["UpdateOffset"] = function(self)
    ...
    if group then
      ...
      while(group) do
        depth = depth + 1
        group = BrAuras.GetData(group).parent
      end
          ...
          onClick = function()
            MenuUtil.CreateContextMenu(UIParent, function(ownerRegion, root)
              root:CreateButton(soundText, function()
                BrAuras.PickDisplay(warning.auraId, tabsForWarning[warning.key] or "information")
              end)
  ...
  ["SetLoaded"] = function(self, prio, file, title, description)
    self:UpdateStatusIcon("load", prio, "Interface\\AddOns\\BrAuras\\Media\\Textures\\" .. file, title, description, nil)
    self:SortStatusIcons()
  end,
  ...
  ["OverrideIcon"] = function(self)
    self.icon:SetTexture("Interface\\Addons\\BrAuras\\Media\\Textures\\icon.blp")
    self.icon:Show()
    if(self.iconRegion and self.iconRegion.Hide) then
      self.iconRegion:Hide();
    end
...
local function Constructor()
  local name = "BrAurasDisplayButton"..AceGUI:GetNextWidgetNum(Type);
  ---@class Button
  ...
  renamebox:SetScript("OnEnterPressed", function()
    ...
    if(newid == "" or (newid ~= oldid and BrAuras.GetData(newid))) then
