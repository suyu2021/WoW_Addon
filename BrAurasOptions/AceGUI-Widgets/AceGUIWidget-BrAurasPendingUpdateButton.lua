...
local Type, Version = "BrAurasPendingUpdateButton", 6
...
local methods = {
  ...
  ["Initialize"] = function(self, id, companionData)
    ...
    function self.callbacks.OnUpdateClick()
      ...
      BrAuras.Import(self.companionData.encoded, nil, nil, linkedAuras)
    end
        ...
        if not self.linkedChildren[auraId] then
          tinsert(
            self.menu,
            {
              text = auraId,
              notCheckable = true,
              func = function() BrAuras.PickDisplay(auraId, "information") end
            }
          )
        end
      end
      LibDD:EasyMenu(self.menu, BrAuras_DropDownMenu, self.followLink, 0, 0, "MENU", 5)
    end
...
local function Constructor()
  local name = "BrAurasPendingUpdateButton" .. AceGUI:GetNextWidgetNum(Type)
  ...
  update:SetNormalTexture([[Interface\AddOns\BrAuras\Media\Textures\wagoupdate_refresh.tga]])
  update:Disable()
  update:SetWidth(24)
  update:SetHeight(24)
  ...
  tex:SetTexture([[Interface\AddOns\BrAuras\Media\Textures\wagoupdate_logo.tga]])
  tex:SetAllPoints()
  updateLogo.tex = tex
