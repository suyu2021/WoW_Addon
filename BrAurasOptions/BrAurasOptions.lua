...
local L = BrAuras.L
local ADDON_NAME = "BrAurasOptions";

local displayButtons = {};
OptionsPrivate.displayButtons = displayButtons;

local spellCache = BrAuras.spellCache;
local savedVars = {};
...
function OptionsPrivate.DuplicateAura(data, newParent, massEdit, targetIndex)
  ...
  while(BrAuras.GetData(new_id)) do
    new_id = base_id .. num
    num = num + 1
  end
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if (event == "ADDON_LOADED") then
    if(addon == ADDON_NAME) then
      db = BrAurasSaved;
      BrAurasOptionsSaved = BrAurasOptionsSaved or {};

      odb = BrAurasOptionsSaved;
...
local function commonParent(controlledChildren)
  ...
  for index, id in ipairs(controlledChildren) do
    local childData = BrAuras.GetData(id);
    local childButton = OptionsPrivate.GetDisplayButton(id)
...
local function CreateNewGroupFromSelection(regionType, resetChildPositions)
  ...
  BrAuras.DeepMixin(data, OptionsPrivate.Private.data_stub)
  data.internalVersion = BrAuras.InternalVersion()
  ...
  if (parent) then
    ...
    BrAuras.Add(parentData);
    OptionsPrivate.Private.AddParents(parentData)
    BrAuras.NewDisplayButton(data);
    BrAuras.UpdateGroupOrders(parentData);
    OptionsPrivate.ClearOptions(parentData.id);
  ...
  for index, childId in pairs(tempGroup.controlledChildren) do
    ...
    local oldParentData = BrAuras.GetData(oldParent)
    if (oldParent) then
      local oldIndex = childButton:GetGroupOrder()

      tremove(oldParentData.controlledChildren, oldIndex)
      BrAuras.Add(oldParentData)
      OptionsPrivate.Private.AddParents(oldParentData)
      BrAuras.UpdateGroupOrders(oldParentData);
      BrAuras.ClearAndUpdateOptions(oldParent);
...
function OptionsPrivate.MultipleDisplayTooltipMenu()
  ...
  local menu = {
    ...
    },
    {
      text = L["Close"],
      notCheckable = 1,
      func = function() BrAuras_DropDownMenu:Hide() end
    }
  };
  ...
  if (allSameParent and commonParent) then
    local parentData = BrAuras.GetData(commonParent);
...
StaticPopupDialogs["BRAURAS_CONFIRM_DELETE"] = {
  text = "",
  button1 = L["Delete"],
  button2 = L["Cancel"],
...
function OptionsPrivate.IsWagoUpdateIgnored(auraId)
    local auraData = BrAuras.GetData(auraId)
      if auraData then
...
function OptionsPrivate.ConfirmDelete(toDelete, parents)
  if toDelete then
    ...
    StaticPopupDialogs["BRAURAS_CONFIRM_DELETE"].text = warningForm:format(#toDelete)
    StaticPopup_Show("BRAURAS_CONFIRM_DELETE", "", "", {toDelete = toDelete, parents = parents})
  end
end
...
if not BrAuras.ToggleOptions then
  ---@type fun(msg: string, Private: Private)
  function BrAuras.ToggleOptions(msg, Private)
    if not Private then
      return
    end
    ...
    elseif (InCombatLockdown()) then
      BrAuras.prettyPrint(L["Options will open after combat ends."])
      reopenAfterCombat = true;
    else
      BrAuras.ShowOptions(msg);
    end
  end
end

function BrAuras.HideOptions()
  if(frame) then
    frame:Hide()
  end
end
...
local function EnsureDisplayButton(data)
  ...
  if not(displayButtons[id]) then
    displayButtons[id] = AceGUI:Create("BrAurasDisplayButton");
    if(displayButtons[id]) then
      displayButtons[id]:SetData(data);
      displayButtons[id]:Initialize();
      displayButtons[id]:UpdateWarning()
    else
      print("|cFF8800FFBrAuras|r: Error creating button for", id);
    end
  end
end
  ...
  local func1 = function()
    ...
    if parents then
      for id in pairs(parents) do
        local parentData = BrAuras.GetData(id)
...
function BrAuras.ShowOptions(msg)
  ...
  if (frame.pickedDisplay) then
    if (OptionsPrivate.IsPickedMultiple()) then
      ...
      BrAuras.PickDisplay(frame.pickedDisplay);
    end
  else
    frame:NewAura();
  end
...
function BrAuras.ClearAndUpdateOptions(id, clearChildren)
...
function BrAuras.NewDisplayButton(data, massEdit)
  local id = data.id;
  OptionsPrivate.Private.ScanForLoads({[id] = true});
  EnsureDisplayButton(db.displays[id]);
  BrAuras.UpdateThumbnail(db.displays[id]);
...
function BrAuras.UpdateGroupOrders(data)
  if(data.controlledChildren) then
...
function OptionsPrivate.SortDisplayButtons(filter, overrideReset, id)
  ...
  if OptionsPrivate.Private.CompanionData.stash then
    for id, companionData in pairs(OptionsPrivate.Private.CompanionData.stash) do
      ...
      if frame.pendingInstallButton:GetExpanded() then
        if not child then
          child = AceGUI:Create("BrAurasPendingInstallButton")
  ...
  if OptionsPrivate.Private.CompanionData.slugs then
    ...
    for id, aura in pairs(BrAurasSaved.displays) do
          ...
          if auraData and auraData.wagoVersion then
            if tonumber(auraData.wagoVersion) > tonumber(version) then
              ...
              if frame.pendingUpdateButton:GetExpanded() then
                ...
                if not child then
                  child = AceGUI:Create("BrAurasPendingUpdateButton")
  ...
  for _, id in ipairs(topLevelLoadedAuras) do
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(BrAuras.GetData(id)) do
...
function BrAuras.PickDisplay(id, tab, noHide)
...
function OptionsPrivate.PickDisplayMultipleShift(target)
  if (frame.pickedDisplay) then
    ...
    if (first and first ~= target) then
      ...
      local firstData = BrAuras.GetData(first);
      local targetData = BrAuras.GetData(target);
      if (firstData.parent == targetData.parent and not targetData.controlledChildren and not firstData.controlledChildren) then
        ...
        if (firstData.parent) then
          local group = BrAuras.GetData(targetData.parent);
          for index, child in ipairs(group.controlledChildren) do
            ...
            if (child == target or child == first) then
              ...
              for i = index + 1, #group.controlledChildren do
                ...
                if (BrAuras.GetData(current).controlledChildren) then
                  -- Skip sub groups
                else
        ...
        elseif (firstData.parent == nil and targetData.parent == nil) then
          ...
          for index, button in ipairs(frame.buttonsScroll.children) do
            if button.type == "BrAurasDisplayButton" then
...
local function CompareButtonOrder(a, b)
  ...
  while(aNode) do
    local parent = BrAuras.GetData(aNode).parent
    if (parent) then
  ...
  while(bNode) do
    local parent = BrAuras.GetData(bNode).parent
    if parent then
...
function OptionsPrivate.DropIndicator()
  ...
  if not indicator then
    ---@class Frame
    indicator = CreateFrame("Frame", "BrAuras_DropIndicator")
    ...
    groupTexture:SetTexture("Interface\\AddOns\\BrAuras\\Media\\Textures\\Square_FullWhite")
...
function BrAuras.UpdateThumbnail(data)
  local id = data.id
...
function OptionsPrivate.OpenModelPicker(baseObject, path)
  if not(C_AddOns.IsAddOnLoaded("BrAurasModelPaths")) then
    local loaded, reason = C_AddOns.LoadAddOn("BrAurasModelPaths");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      BrAuras.prettyPrint(string.format(L["ModelPaths could not be loaded, the addon is %s"], reason));
      BrAuras.ModelPaths = {};
    end
    OptionsPrivate.ModelPicker(frame).modelTree:SetTree(BrAuras.ModelPaths)
  end
...
function OptionsPrivate.OpenTriggerTemplate(data, targetId)
  if not(C_AddOns.IsAddOnLoaded("BrAurasTemplates")) then
    local loaded, reason = C_AddOns.LoadAddOn("BrAurasTemplates");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      BrAuras.prettyPrint(string.format(L["Templates could not be loaded, the addon is %s"], reason));
      return;
    end
    frame.newView = BrAuras.CreateTemplateView(OptionsPrivate.Private, frame);
  end
...
function OptionsPrivate.UpdateTextReplacements(frame, data)
  ...
  -- Create a modified BrAurasSnippetButton for each property and add it to ScrollList
  ...
  for i, prop in ipairs(sortedProps) do
    if prop.type == "header" then
      ...
      local button = AceGUI:Create("BrAurasSnippetButton")
...
function BrAuras.SetMoverSizer(id)
...
function BrAuras.GetMoverSizerId()
...
function BrAuras.NewAura(sourceData, regionType, targetId)
  local function ensure(t, k, v)
    return t and k and v and t[k] == v
  end
  local new_id = OptionsPrivate.Private.FindUnusedId("New")
  local data = {id = new_id, regionType = regionType, uid = BrAuras.GenerateUniqueID()}
  BrAuras.DeepMixin(data, OptionsPrivate.Private.data_stub);
  if (sourceData) then
    BrAuras.DeepMixin(data, sourceData);
  end
  data.internalVersion = BrAuras.InternalVersion();
  OptionsPrivate.Private.validate(data, OptionsPrivate.Private.regionTypes[regionType].default);
  ...
  if targetId then
    ...
    if (target) then
      ...
      if (group) then
        ...
        BrAuras.Add(data);
        BrAuras.Add(group.data);
        OptionsPrivate.Private.AddParents(group.data)
        BrAuras.NewDisplayButton(data);
        BrAuras.UpdateGroupOrders(group.data);
      ...
      error(string.format("Calling 'BrAuras.NewAura' with invalid groupId %s. Reload your UI to fix the display list.", targetId))
    end
  else
    -- move source into the top-level list
    BrAuras.Add(data);
