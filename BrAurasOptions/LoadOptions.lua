...
local function CorrectSpellName(input)
  ...
  if(inputId) then
    return inputId
  elseif BrAuras.IsClassicEra() and input then
...
function OptionsPrivate.ConstructOptions(prototype, data, startorder, triggernum, triggertype)
  ...
  for index, arg in pairs(prototype.args) do
    ...
    elseif name and (hiddenProperty == "hidden" or not arg.hidden) then
      ...
      elseif (arg.type == "header") then
        ...
        options["use_"..name] = {
          type = "toggle",
          width = arg.width or BrAuras.normalWidth,
          name = arg.display,
          order = order,
          hidden = hidden,
          desc = arg.desc,
      ...
      if(arg.type == "toggle" or arg.type == "tristate") then
        options["use_"..name].width = arg.width or BrAuras.doubleWidth;
      end
      if(arg.type == "spell" or arg.type == "aura" or arg.type == "item") then
        if not arg.showExactOption then
          options["use_"..name].width = (arg.width or BrAuras.normalWidth) - 0.2;
        end
      end
      ...
      for entryNumber = 1, countEntries + 1 do
        ...
        if(arg.type == "number") then
          if entryNumber > 1 then
            options["spacer_"..name..suffix].width = BrAuras.normalWidth
          end
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.noOperator and BrAuras.normalWidth or BrAuras.halfWidth,
        ...
        elseif(arg.type == "string" or arg.type == "tristatestring") then
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.multiline and BrAuras.doubleWidth or BrAuras.normalWidth,
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
            ...
            control = arg.multiline and "BrAuras-MultiLineEditBoxWithEnter" or nil,
            get = function()
        ...
        elseif(arg.type == "longstring") then
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.canBeCaseInsensitive and BrAuras.normalWidth or BrAuras.doubleWidth,
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
        ...
        elseif(arg.type == "spell" or arg.type == "aura" or arg.type == "item") then
          if entryNumber > 1 then
            options["spacer_"..name..suffix].width = BrAuras.normalWidth - (arg.showExactOption and 0 or 0.2)
          end
          local disabled = not trigger["use_"..realname]
          options[name..suffix.."dummy"] = {
            type = "description",
            ...
            image = function()
              ...
              if type(value) == "number" or type(value) == "string" then
                ...
                elseif(arg.type == "spell") then
                  if arg.negativeIsEJ and BrAuras.IsRetail() then
                    local key = BrAuras.SafeToNumber(value)
          ...
          options[name..suffix] = {
            type = "input",
            width = (arg.showExactOption and BrAuras.doubleWidth or BrAuras.normalWidth) - (arg.showExactOption and 0.2 or 0),
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
            get = function()
              ...
              elseif(arg.type == "spell") then
                ...
                if value and value ~= "" and (type(value) == "number" or type(value) == "string") then
                  local spellID = BrAuras.SafeToNumber(value)
                  if spellID then
                    if arg.negativeIsEJ and BrAuras.IsRetail() and spellID < 0 then
            ...
            set = function(info, v)
              ...
              if not arg.noValidation then
                if(arg.type == "aura") then
                  fixedInput = BrAuras.spellCache.CorrectAuraName(v);
                elseif(arg.type == "spell") then
        ...
        elseif(arg.type == "select" or arg.type == "unit" or arg.type == "currency") then
          ...
          if(arg.type == "unit" or arg.type == "currency") then
            ...
            options["specific_"..name..suffix] = {
              ...
              desc = specificDesc,
              order = order,
              validate = arg.type == "currency" and BrAuras.ValidateNumeric or false,
  ...
  if prototype.countEvents then
    options.use_count = {
      type = "toggle",
      width = BrAuras.normalWidth,
      name = BrAuras.newFeatureString .. L["Count"],
      order = order,
      get = function()
  ...
  if prototype.delayEvents then
    options.use_delay = {
      type = "toggle",
      width = BrAuras.normalWidth,
      name = BrAuras.newFeatureString .. L["Delay"],
      order = order,
      get = function()
    ...
    if disabled then
      ...
      options.delay = {
        ...
        validate = BrAuras.ValidateTime,
        get = function()
          return OptionsPrivate.Private.tinySecondFormat(trigger.delay)
        end,
        set = function(info, v)
          trigger.delay = BrAuras.TimeToSeconds(v)
          BrAuras.Add(data)
        end
      };
      order = order + 1;
  ...
  if prototype.timedrequired then
    ...
    options.duration = {
      ...
      set = function(info, v)
        trigger.duration = tostring(BrAuras.TimeToSeconds(v))
        BrAuras.Add(data)
      end
    }
    order = order + 1;
