...
local function ConstructMover(frame)
  ...
  arrowTexture:SetTexture("Interface\\Addons\\BrAuras\\Media\\Textures\\offscreen.tga")
  ...
  UpdateHighlight = function(self)
    if self.highlight then
      for _, data in ipairs(self.references) do
        local region = BrAuras.GetRegion(data.id)
...
AlignmentLines.CreateLineInformation = function(self, data, sizerPoint)
  ...
  for id, v in pairs(OptionsPrivate.displayButtons) do
    local region = BrAuras.GetRegion(v.data.id)
    if not skipIds[id]
...
AlignmentLines.CreateLines = function(self, data, sizerPoint)
  ...
  local align = (BrAurasOptionsSaved.magnetAlign and not IsShiftKeyDown())
                    or (not BrAurasOptionsSaved.magnetAlign and IsShiftKeyDown())
  if align then
