...
if not BrAurasSaved.editor_tab_spaces then BrAurasSaved.editor_tab_spaces = 4 end
if not BrAurasSaved.editor_font_size then BrAurasSaved.editor_font_size = 12 end -- set default font size if missing
local color_scheme = {[0] = "|r"}
local function set_scheme()
  if not BrAurasSaved.editor_theme then
    BrAurasSaved.editor_theme = "Monokai"
  end
  local theme = editor_themes[BrAurasSaved.editor_theme]
...
local function ConstructTextEditor(frame)
  ...
  if (fontPath) then
    editor.editBox:SetFont(fontPath, BrAurasSaved.editor_font_size, "")
  end
  ...
  IndentationLib.enable(editor.editBox, color_scheme, BrAurasSaved.editor_tab_spaces)
    ...
    if level == 1 then
      for k, v in pairs(editor_themes) do
        local item = {
          ...
          isNotRadio = false,
          checked = function()
            return BrAurasSaved.editor_theme == k
          end,
          func = function()
            BrAurasSaved.editor_theme = k
            set_scheme()
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          ...
          isNotRadio = true,
          checked = function()
            return BrAurasSaved.editor_bracket_matching
          end,
          func = function()
            BrAurasSaved.editor_bracket_matching = not BrAurasSaved.editor_bracket_matching
          end
        },
      level)
      LibDD:UIDropDownMenu_AddButton(
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          text = BrAuras.newFeatureString .. L["Font Size"],
          hasArrow = true,
          notCheckable = true,
          menuList = "sizes"
        },
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return BrAurasSaved.editor_tab_spaces == i
            end,
            func = function()
              BrAurasSaved.editor_tab_spaces = i
              IndentationLib.enable(editor.editBox, color_scheme, BrAurasSaved.editor_tab_spaces)
              editor.editBox:SetText(editor.editBox:GetText().."\n")
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return BrAurasSaved.editor_font_size == i
            end,
            func = function()
              BrAurasSaved.editor_font_size = i
              editor.editBox:SetFont(fontPath, BrAurasSaved.editor_font_size, "")
            end
          },
        level)
  ...
  BrAurasOptionsSaved.savedSnippets = BrAurasOptionsSaved.savedSnippets or {}
  local savedSnippets = BrAurasOptionsSaved.savedSnippets
  ...
  local snippetsFrame = CreateFrame("Frame", "BrAurasSnippets", group.frame, "PortraitFrameTemplate")
  ...
  apiSearchFrame = CreateFrame("Frame", "BrAurasAPISearchFrame", group.frame, "PortraitFrameTemplate")
  ...
  -- filter line
  local filterInput = CreateFrame("EditBox", "BrAurasAPISearchFilterInput", apiSearchFrame, "SearchBoxTemplate")
  ...
  editor.editBox:HookScript(
    "OnChar",
    function(_, char)
      if not IsControlKeyDown() and BrAurasSaved.editor_bracket_matching then
        if char == "(" then
