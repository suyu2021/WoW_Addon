...
local function Diff(ours, theirs)
  ...
  -- generates a diff which BrAuras.Update can use
  local debug = false
...
local function BuildUidMap(data, children, type)
  ...
  local function handle(data)
    ...
    if data.anchorFrameType == "SELECTFRAME"
     ...
     and data.anchorFrameFrame:sub(1, 10) == "BrAuras:"
  then
  ...
  uidMap.GetPhase2Data = function(self, uid, withAppliedPath, activeCategories)
    ...
    if self.map[uid].anchorFrameFrame then
      ...
      if target then
        data.anchorFrameFrame = "BrAuras:" .. target
      end
    end
  ...
  uidMap.EnsureUniqueIdOfUnmatched = function(self, uid, IncProgress)
    ...
    if not self:GetUIDMatch(uid) then
      if OptionsPrivate.Private.GetDataByUID(uid) then
        local newUid = BrAuras.GenerateUniqueID()
...
local function AddAuraList(container, uidMap, list, categories, expandText)
  local expand = AceGUI:Create("BrAurasExpand")
  local collapsed = true
  local image = collapsed and "Interface\\AddOns\\BrAuras\\Media\\Textures\\expand"
                           or "Interface\\AddOns\\BrAuras\\Media\\Textures\\collapse"
  ...
  local auraLabelContainer = AceGUI:Create("BrAurasInlineGroup")
...
local methods = {
  Open = function(self, data, children, target, linkedAuras, sender, callbackFunc)
    ...
    do
      ...
      if (highestVersion > BrAuras.InternalVersion()) then
        ...
        highestVersionWarning:SetText(L["This aura was created with a newer version of BrAuras.\nUpgrade your version of BrAuras or wait for next release before installing this aura."])
    ...
    if matchInfo ~= nil then
      if not hasChanges(matchInfo) then
        ...
        self:AddChild(AceGUI:Create("BrAurasSpacer"))
        ...
        local updateUiArea = AceGUI:Create("BrAurasInlineGroup")
    ...
    local currentBuild = floor(BrAuras.BuildInfo / 10000)
  ...
  CreateUpdateArea = function(self, area, matchInfo)
    area:AddChild(AceGUI:Create("BrAurasSpacer"))
  ...
  Import = function(self)
    if BrAuras.IsClassicEra() and C_GameRules.IsHardcoreActive() then
      StaticPopupDialogs["BRAURAS_CONFIRM_IMPORT_HARDCORE"] = {
      ...
      StaticPopup_Show("BRAURAS_CONFIRM_IMPORT_HARDCORE")
      return
    end
    OptionsPrivate.Private.Threads:Add("import", coroutine.create(function()
      self:ImportImpl()
  ...
  ImportImpl = function(self)
    ...
    if pendingPickData then
      ...
      BrAuras.PickDisplay(pendingPickData.id, pendingPickData.tabToShow)
    end
  ...
  -- Note: There's a final renaming via BrAuras.Rename at the end of the update process
  FixUpNames = function(self, uidMap, uid)
    ...
    if existingData then
      ...
      if BrAuras.GetData(uidMap:GetIdFor(uid)) then
  ...
  UpdatePhase1 = function(self, structureUidMap, uid, GetPhase1Data, phase2Order)
    ...
    BrAuras.Add(data)
    BrAuras.NewDisplayButton(data, true)
    self:IncProgress10()
  ...
  UpdatePhase2 = function(self, structureUidMap, GetPhase2Data, phase2Order, copies)
    for i = #phase2Order, 1, -1 do
      ...
      BrAuras.UpdateGroupOrders(data)
  ...
  AddProgressWidgets = function(self)
    ...
    local progress = AceGUI:Create("BrAurasProgressBar")
