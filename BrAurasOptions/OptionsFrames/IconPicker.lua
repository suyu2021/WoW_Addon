...
local function ConstructIconPicker(frame)
  ...
  local function iconPickerFill(subname, doSort)
    ...
    local AddButton = function(name, icon)
      local button = AceGUI:Create("BrAurasIconButton");
      button:SetName(name);
  ...
  local input = CreateFrame("EditBox", "BrAurasFilterInput", group.frame, "SearchBoxTemplate")
  ...
  local icon = AceGUI:Create("BrAurasIconButton");
  icon.frame:Disable();
  ...
  function group.Pick(self, texturePath)
    ...
    if self.groupIcon then
      ...
      BrAuras.Add(self.baseObject)
      BrAuras.ClearAndUpdateOptions(self.baseObject.id)
      BrAuras.UpdateThumbnail(self.baseObject)
    else
  ...
  function group.CancelClose()
    ...
    if group.groupIcon then
      ...
      BrAuras.Add(group.baseObject)
      BrAuras.ClearAndUpdateOptions(group.baseObject.id)
      BrAuras.UpdateThumbnail(group.baseObject)
    else
