...
local function getAuraMatchesLabel(name)
  local ids = BrAuras.spellCache.GetSpellsMatching(name)
  if ids then
    local numMatches = 0
...
local function getAuraMatchesList(name, showSpellIdRecommendation)
  ...
  if ids then
    ...
    for id, _ in pairs(ids) do
      ...
      if BrAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(id) then
    ...
    if showSpellIdRecommendation then
      local tip = L["BrAuras recommends using spell ids instead of names. Spell ids are automatically localized."]
  ...
  for i = 1, size do
    ...
    if isExactSpellId then
      aura_options[iconOption].name = function()
        return OptionsPrivate.Private.ExecEnv.GetSpellName(BrAuras.SafeToNumber(trigger[optionKey] and trigger[optionKey][i]) or "")
      end
      ...
      aura_options[iconOption].name = function()
        local spellId = trigger[optionKey] and trigger[optionKey][i] and BrAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
      ...
      aura_options[iconOption].desc = function()
        ...
        local spellId = input and BrAuras.SafeToNumber(input)
        if spellId then
      ...
      aura_options[iconOption].image = function()
        ...
        elseif input and input ~= "" then
          icon = "Interface\\AddOns\\BrAuras\\Media\\Textures\\info"
        end
        return icon and tostring(icon) or "", 18, 18
      end
      ...
      aura_options[iconOption].func = function()
        local input = trigger[optionKey] and trigger[optionKey][i]
        local spellId = input and BrAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
          -- Do nothing
        elseif input and input ~= "" then
          ...
          if bestSuggestion then
            ...
            BrAuras.Add(data)
            BrAuras.ClearAndUpdateOptions(data.id)
          end
        end
      end
    end
    ...
    aura_options[prefix .. i] = {
      ...
      get = function(info)
        ...
        local spellName, _, _, _, _, _, spellID = OptionsPrivate.Private.ExecEnv.GetSpellInfo(BrAuras.SafeToNumber(rawString))
        if spellName and spellID then
          return ("%s (%s)"):format(spellID, spellName) .. "\0" .. rawString
        elseif BrAuras.SafeToNumber(rawString) then
      ...
      set = function(info, v)
        ...
        if v == "" then
          ...
          if isExactSpellId then
            ...
            local _, spellId = BrAuras.spellCache.CorrectAuraName(v)
            if spellId then
      ...
      validate = isExactSpellId and BrAuras.ValidateNumeric or nil,
      control = "BrAurasInputFocus",
    }
  end
  -- VALIDATE ?
end
...
local function GetBuffTriggerOptions(data, triggernum)
  ...
  local ValidateNumeric = BrAuras.ValidateNumeric
  local aura_options = {
    useUnit = {
      type = "toggle",
      width = BrAuras.normalWidth,
    ...
    warnSpecifcUnit = {
      ...
      order = 10.4,
      hidden = function() return not (trigger.type == "aura2" and trigger.unit == "member" and BrAuras.UntrackableUnit(trigger.specificUnit)) end
    },
    warnSoftTarget = {
      type = "description",
      width = BrAuras.doubleWidth,
      ...
      order = 10.4,
      hidden = function() return not BrAuras.IsUntrackableSoftTarget(trigger.unit) end
    },
    useDebuffType = {
      type = "toggle",
      width = BrAuras.normalWidth,
    ...
    useActualSpec = {
      ...
      desc = L["Requires LibSpecialization, that is e.g. a up-to date BrAuras version"],
      order = 66.3,
      hidden = function() return
        not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        and BrAuras.IsCataOrMistsOrRetail())
      end,
    },
    actualSpec = {
      type = "multiselect",
    ...
    useArenaSpec = {
      ...
      order = 67.8,
      hidden = function() return
        not (BrAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena")
      end
    },
    arena_spec = {
      type = "multiselect",
      ...
      values = OptionsPrivate.Private.spec_types_all,
      hidden = function()
        return not (BrAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and trigger.useArenaSpec)
      end,
      order = 67.9
    },
    arena_specSpace = {
      ...
      width = BrAuras.normalWidth,
      hidden = function()
        return not (BrAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and not trigger.useArenaSpec)
      end,
    },
    ...
    inRange = {
      ...
      order = 69.81,
      width = BrAuras.doubleWidth,
      hidden = function() return not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party") and BrAuras.IsRetail()) end
    },
    ignoreInvisible = {
      type = "toggle",
...
BrAuras.RegisterTriggerSystemOptions({"aura2"}, GetBuffTriggerOptions)
