...
typeControlAdders = {
  ...
  group = function(options, args, data, order, prefix, i)
    ...
    args[prefix .. "noMerge"] = {
      type = "toggle",
      name = BrAuras.newFeatureString .. name(option, "noMerge", L["Prevent Merging"]),
      desc = desc(option, "noMerge", L["If checked, then this group will not merge with other group when selecting multiple auras."]),
      order = order(),
      width = option.groupType =="simple" and BrAuras.doubleWidth or BrAuras.normalWidth,
    ...
    if option.groupType ~="simple" then
      args[prefix .. "sortAlphabetically"] = {
        type = "toggle",
        name = BrAuras.newFeatureString .. name(option, "sortAlphabetically", L["Sort"]),
...
function addAuthorModeOption(options, args, data, order, prefix, i)
  ...
  args[prefix .. "collapse"] = {
    ...
    order = order(),
    width = BrAuras.doubleWidth - buttonWidth,
    func = function()
      for id, optionData in pairs(option.references) do
    ...
    image = collapsed and "Interface\\AddOns\\BrAuras\\Media\\Textures\\expand" or
      "Interface\\AddOns\\BrAuras\\Media\\Textures\\collapse",
    imageWidth = 18,
    imageHeight = 18,
  ...
  args[prefix .. "upAndIn"] = {
    ...
    image = "Interface\\AddOns\\BrAuras\\Media\\Textures\\upright",
    imageWidth = 24,
    imageHeight = 24,
    control = "BrAurasIcon"
  }
  args[prefix .. "downAndIn"] = {
    ...
    image = "Interface\\AddOns\\BrAuras\\Media\\Textures\\downright",
    imageWidth = 24,
    imageHeight = 24,
    control = "BrAurasIcon"
  }
  ...
  args[prefix .. "upAndOut"] = {
    ...
    func = function()
      for id, optionData in pairs(option.references) do
        ...
        BrAuras.Add(optionData.data)
      end
      BrAuras.ClearAndUpdateOptions(data.id, true)
    end,
    image = "Interface\\AddOns\\BrAuras\\Media\\Textures\\upleft",
    imageWidth = 24,
    imageHeight = 24,
    control = "BrAurasIcon"
  }
  args[prefix .. "downAndOut"] = {
    ...
    image = "Interface\\AddOns\\BrAuras\\Media\\Textures\\downleft",
    imageWidth = 24,
    imageHeight = 24,
    control = "BrAurasIcon"
  }
...
local function addUserModeOption(options, args, data, order, prefix, i)
  ...
  if optionClass == "simple" then
    userOption = {
      ...
      width = (option.width or 1) * BrAuras.normalWidth,
      order = order(),
      get = getUser(option),
    ...
    if not collapsed then
      ...
      if option.groupType == "array" then
        ...
        args[prefix .. "resetEntry"] = {
          ...
          width = 0.15,
          image = "Interface\\Addons\\BrAuras\\Media\\Textures\\reset",
          imageWidth = 18,
          imageHeight = 18,
          control = "BrAurasIcon"
        }
        if option.limitType ~= "fixed" then
          args[prefix .. "createEntry"] = {
            ...
            width = 0.15,
            image = "Interface\\AddOns\\BrAuras\\Media\\Textures\\add",
            imageWidth = 18,
            imageHeight = 18,
            control = "BrAurasIcon"
          }
